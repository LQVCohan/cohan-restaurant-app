...
test.describe("guest smoke: home and food detail", () => {
  test("guest can open home, see public menu content, and visit a food detail without customer-only UI", async ({ page }) => {
    const pageErrors = [];
    ...
    await expect(page.getByRole("link", { name: /Hồ sơ|Đăng xuất/i })).toHaveCount(0);
