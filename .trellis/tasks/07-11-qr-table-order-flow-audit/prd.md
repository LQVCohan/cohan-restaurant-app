...
3. Idempotency chỉ kiểm tra trước transaction nên hai request đồng thời có thể cùng đi qua bước kiểm tra trước khi một request commit.
...
- Dùng chính bản ghi `table_session` đang hoạt động làm điểm khóa tuần tự; sau khi transaction cạnh tranh được retry, kiểm tra lại idempotency key trước khi tạo batch. Cách này tái sử dụng unique active-session hiện có và không cần thêm index/migration mới.
...
- Bổ sung regression tests cho transaction sequencing, concurrent retry, capability và realtime payload.
