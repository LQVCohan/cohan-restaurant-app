...
1. **The manager shell can keep visual state from the previous account.** The shared `AuthProvider` already clears Apollo on logout and rejects stale refresh/`Me` results by session epoch and account ID. Header, Sidebar and account-center local state can still survive an identity change, leaving an old avatar failure state, open profile dialog or stale dropdown visible until reload.
...
4. **There is no complete menu-and-dish overview.** The current menu page can manage one selected time slot, while managers still lack one view that groups all menu frames and their dishes. The backend already supports manager-scoped `menus` and `menuItemsConnection` queries grouped by time slot.

## End-to-end flows traced

- `User / refresh session -> login and Me operations -> AuthProvider -> AuthContext -> Header / Sidebar / account center`.
...
- Reset Header, Sidebar and account-center UI state whenever the authenticated account ID changes.
...
1. Switching from account A to account B closes account-A overlays, resets avatar-local state and immediately renders account B's name, avatar and profile data without browser refresh.
