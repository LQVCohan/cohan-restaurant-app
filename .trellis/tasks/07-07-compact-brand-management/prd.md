...
6. `ManagerLayout` mounts the page for `manager#brands`.

The data contract and authorization path are correct. The root cause is page composition and spacing, so no backend or GraphQL change is required.
...
- Keep the add-member controls compact on one desktop row when possible.
...
- Account, role, manager branch scope, and add action fit one row at the reported desktop width.
