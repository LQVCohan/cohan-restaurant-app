...
- Customer checkout sends a modal-generated attempt value, then the Apollo idempotency link replaces it with a key stored by payload fingerprint.
...
Checkout and wallet settlement do not use one explicit idempotency contract. The browser key lifecycle is tied to payload storage rather than the checkout attempt, while the wallet service treats the key as sufficient without binding it to a canonical wallet-payment fingerprint.
...
- Scope the stable browser key to the checkout attempt value already supplied by the modal, with payload fingerprint only as a fallback for other callers.
...
1. Retrying the same checkout attempt sends the same actual key.
2. Reopening checkout for an identical cart can create a new key and a new legitimate checkout.
...
6. A repeated key with different order IDs returns `IDEMPOTENCY_KEY_REUSED` and never debits the wallet.
