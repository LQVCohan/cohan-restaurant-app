...
- Adds UI-side status labeling so overdue checked-in records display as `Thiếu check-out` even where the existing resolver still derives status from timestamps.
