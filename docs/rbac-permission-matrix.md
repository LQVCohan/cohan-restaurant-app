...
Customer-facing menu, promotion, coupon, and voucher package browsing remains public for active/available records only. Internal reads that include inactive, hidden, unavailable, expired, or administrative records require the corresponding module read permission. Customer-owned flows such as creating reservations and viewing/managing the customer's own reservations remain separate from staff/manager restaurant permission checks.
