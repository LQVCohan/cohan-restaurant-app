...
const readableMessage = (value, type) => {
  ...
  if (text.includes("Variable") || text.includes("ValidationError")) {
  ...
  return useMemo(
    () => ({
      ...
      showNotification: (message, type, duration) =>
        ctx.showNotification(readableMessage(message, type), type, duration),
    }),
    [ctx],
  );
};
