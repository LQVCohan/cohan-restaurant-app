...
const syncMapFromInputs = (card, context, { recenter = false } = {}) => {
  ...
  if (!state.marker) {
    const markerOptions = RESTAURANT_MARKER_ICON
      ? { draggable: true, icon: RESTAURANT_MARKER_ICON }
      : { draggable: true };
    state.marker = L.marker([pair.lat, pair.lng], markerOptions).addTo(state.map);
