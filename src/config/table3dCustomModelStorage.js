...
export const loadCustomTableModels = (scope = "default") => {
  ...
  try {
    const raw = window.localStorage.getItem(getCustomTableModelStorageKey(scope));
    if (!raw) return [];
...
export const saveCustomTableModels = (models, scope = "default") => {
  ...
  try {
    window.localStorage.setItem(getCustomTableModelStorageKey(scope), JSON.stringify(deduped));
    return deduped;
  } catch {
    return deduped;
  }
...
export const upsertCustomTableModel = (model, scope = "default") => {
  ...
  return saveCustomTableModels([{ ...normalized, updatedAt: new Date().toISOString() }, ...current], scope);
};
...
export const mergeCatalogWithCustomModels = (catalogModels = [], customModels = []) => {
  ...
  const safeCustom = custom.filter((item) => item && !catalogKeys.has(item.key));
