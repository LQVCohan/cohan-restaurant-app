...
const queryMock = () => ({
  ...
  result: { data: { staffAttendanceRecords: records, attendanceCorrectionRequests: corrections, overtimeRequests } },
});
...
describe("StaffAttendancePage", () => {
  it("renders attendance stats and request forms", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Ngày làm việc"), { target: { value: "2026-06-30" } });
    ...
    expect(screen.getByRole("heading", { name: "Gửi yêu cầu chỉnh công" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Gửi yêu cầu tăng ca" })).toBeInTheDocument();
    expect(screen.getByText("5.08")).toBeInTheDocument();
  });

  it("submits attendance correction with the selected shift/timesheet payload", async () => {
    const mutationResult = vi.fn(() => ({
      data: { createAttendanceCorrectionRequest: { id: "acr-2", status: "pending", workDate: records[0].workDate, correctionType: "wrong_check_out", requestedCheckInAt: null, requestedCheckOutAt: "2026-06-30T22:45:00.000+07:00", reason: "Sai giờ ra thực tế", requestedAt: "2026-06-30T23:10:00.000+07:00" } },
    }));
    renderPage([
      queryMock(),
      {
    ...
    fireEvent.change(screen.getByLabelText("Loại chỉnh công"), { target: { value: "wrong_check_out" } });
    fireEvent.change(screen.getByLabelText("Giờ ra đề xuất"), { target: { value: "22:45" } });
    fireEvent.change(screen.getAllByLabelText("Lý do")[0], { target: { value: "Sai giờ ra thực tế" } });
  ...
  it("submits overtime and can cancel pending requests", async () => {
    const overtimeResult = vi.fn(() => ({
      data: { createOvertimeRequest: { id: "ot-2", status: "pending_approval", workDate: dateVariables.startDate, plannedStartTime: "2026-06-30T22:15:00.000+07:00", plannedEndTime: "2026-06-30T23:15:00.000+07:00", plannedOvertimeMinutes: 60, overtimeType: "weekday", reason: "Dọn kho cuối ca", requestedAt: "2026-06-30T21:45:00.000+07:00" } },
    }));
    const cancelCorrectionResult = vi.fn(() => ({ data: { cancelAttendanceCorrectionRequest: { id: "acr-1", status: "cancelled" } } }));
    const cancelOvertimeResult = vi.fn(() => ({ data: { cancelOvertimeRequest: { id: "ot-1", status: "cancelled" } } }));
    ...
    renderPage([
      ...
      queryMock(),
      { request: { query: CANCEL_ATTENDANCE_CORRECTION, variables: { id: "acr-1" } }, result: cancelCorrectionResult },
      queryMock(),
      { request: { query: CANCEL_OVERTIME_REQUEST, variables: { id: "ot-1" } }, result: cancelOvertimeResult },
      queryMock(),
    ]);

    fireEvent.change(screen.getByLabelText("Ngày làm việc"), { target: { value: "2026-06-30" } });
    await screen.findByText("Ca tối");
    fireEvent.change(screen.getByLabelText("Bắt đầu tăng ca"), { target: { value: "22:15" } });
    fireEvent.change(screen.getByLabelText("Kết thúc tăng ca"), { target: { value: "23:15" } });
    fireEvent.change(screen.getAllByLabelText("Lý do")[1], { target: { value: "Dọn kho cuối ca" } });
