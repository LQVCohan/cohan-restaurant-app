...
const LoginAudience = () => (
  <div
    className="auth-audience-grid"
    aria-label="Nền tảng cho khách hàng và thương hiệu"
  >
    <span>
      <strong>Khách hàng</strong>
      <small>Đặt món, đặt bàn, khẩu vị và tiện ích</small>
    </span>
    <span>
      <strong>Thương hiệu</strong>
      <small>Thương hiệu, chi nhánh, POS, quản lý</small>
    </span>
  </div>
);
...
function resolveRegisterErrorMessage(error) {
  ...
  if (
    ...
    return "Email hoặc thương hiệu này đã được sử dụng.";
...
const LoginPage = () => {
  ...
  const [brandRegisterForm, setBrandRegisterForm] = useState(
    BRAND_REGISTER_INITIAL,
  );
  ...
  const [loginWithGoogle, { loading: googleLoginLoading }] = useMutation(
    LOGIN_WITH_GOOGLE_MUTATION,
    {
      ...
      onCompleted: (data) => {
        ...
        showNotification(
          `Chào mừng ${loggedInUser.fullName || loggedInUser.email}.`,
          "success",
        );
      },
    },
  ...
  const handleGoogleCredential = useCallback(
    (response) => {
      ...
      if (!idToken) {
        showNotification(
          "Google không trả về mã đăng nhập. Vui lòng thử lại.",
          "error",
        );
        return;
      }
  ...
  useEffect(() => {
    if (!GOOGLE_CLIENT_ID || mobileMode !== "login" || isRightPanelActive)
      return undefined;
    let cancelled = false;
    ...
    loadGoogleIdentityScript()
      ...
      .catch(() => {
        if (!cancelled)
          showNotification(
            "Không tải được Google Sign-In. Vui lòng thử lại sau.",
            "error",
          );
      });
  ...
  const [registerBusinessOwner, { loading: brandRegisterLoading }] =
    useMutation(REGISTER_BUSINESS_OWNER, {
      errorPolicy: "none",
      onError: (error) =>
        showNotification(resolveRegisterErrorMessage(error), "error"),
      onCompleted: (data) => {
        ...
        if (!payload?.accessToken || !payload?.user) {
          showNotification(
            "Đăng ký thương hiệu thành công. Vui lòng đăng nhập để tiếp tục.",
            "success",
          );
          togglePanel(false);
          return;
        ...
        showNotification(
          `Đã tạo thương hiệu ${payload.brand?.name || "mới"}.`,
          "success",
        );
  ...
  const handleBrandRegister = () => {
    ...
    if (!fullName || !email || !brandName || !brandRegisterForm.password)
      return showNotification(
        "Vui lòng nhập người đại diện, email, mật khẩu và tên thương hiệu",
        "warning",
      );
    if (brandRegisterForm.password !== brandRegisterForm.confirmPassword)
      return showNotification("Mật khẩu xác nhận không khớp", "error");
    if (!brandRegisterForm.terms)
      return showNotification(
        "Bạn cần đồng ý với chính sách & điều khoản",
        "warning",
      );
    ...
    registerBusinessOwner({
      variables: {
        input: {
          ...
          firstRestaurantAddress: address
            ? { line1: address, country: "Vietnam" }
            : undefined,
        },
      },
    });
      ...
      <div className="captcha-container">
        {shouldRenderCaptcha ? (
          ...
          <p className="captcha-message captcha-message--danger">
            Captcha chưa được cấu hình. Vui lòng kiểm tra
  ...
  const renderRegisterModeSwitch = () => (
    <div
      ...
      role="tablist"
      aria-label="Chọn loại tài khoản đăng ký"
    >
      <button
        type="button"
        role="tab"
        aria-selected={registerMode === "customer"}
        className={registerMode === "customer" ? "active" : ""}
        onClick={() => switchRegisterMode("customer")}
      >
        Khách hàng
      </button>
      <button
        type="button"
        role="tab"
        aria-selected={registerMode === "brand"}
        className={registerMode === "brand" ? "active" : ""}
        onClick={() => switchRegisterMode("brand")}
      >
        Thương hiệu
      </button>
    </div>
  ...
  const submitLoading = isBrandRegister
    ? brandRegisterLoading
    : registerLoading;
  ...
  return (
    <main className="login-page-wrapper" aria-labelledby="login-form-title">
      ...
      <div
        ...
        role="tablist"
        aria-label="Chuyển chế độ đăng nhập"
      >
        <button
          type="button"
          role="tab"
      ...
      <section
        ...
        id="container"
        aria-label="Đăng nhập và đăng ký Cohan"
      >
        <div
          id="register-panel"
        ...
        >
          <form
            className={`auth-form auth-form--register ${isBrandRegister ? "auth-form--brand-register" : ""}`}
          ...
          >
            ...
            {isBrandRegister ? (
              <>
                <p className="auth-subtitle">
                  Tạo thương hiệu trước, sau đó hệ thống sẽ tạo chi nhánh đầu
                  tiên cho thương hiệu đó.
                </p>
                <label
                  className="field-label"
                  htmlFor="brand-register-full-name"
                >
                  Người đại diện
                </label>
                <div className="input-wrapper">
                  <FieldIcon type="user" />
                  <input
                    id="brand-register-full-name"
                    type="text"
                    placeholder="Họ và tên"
                    autoComplete="name"
                    value={brandRegisterForm.fullName}
                    onChange={(e) =>
                      setBrandRegisterForm({
                        ...brandRegisterForm,
                ...
                <label className="field-label" htmlFor="brand-register-email">
                  Email quản trị
                </label>
                <div className="input-wrapper">
                  <FieldIcon type="mail" />
                ...
                <label className="field-label" htmlFor="brand-register-phone">
                  Số điện thoại
                </label>
                <div className="input-wrapper">
                  <FieldIcon type="phone" />
                  <input
                    id="brand-register-phone"
                    type="tel"
                    placeholder="Số điện thoại"
                    autoComplete="tel"
                    value={brandRegisterForm.phone}
                    onChange={(e) =>
                      setBrandRegisterForm({
                        ...brandRegisterForm,
                ...
                <label
                  className="field-label"
                  htmlFor="brand-register-password"
                >
                  Mật khẩu
                </label>
                <div className="password-field">
                  <div className="input-wrapper">
                    ...
                    <input
                      ...
                      type={
                        registerPasswordVisibility.brand ? "text" : "password"
                      }
                      placeholder="Mật khẩu"
                      autoComplete="new-password"
                      aria-describedby="brand-register-password-strength"
                      value={brandRegisterForm.password}
                      onChange={(e) =>
                        setBrandRegisterForm({
                          ...brandRegisterForm,
                    ...
                    <PasswordToggleButton
                      visible={registerPasswordVisibility.brand}
                      onToggle={() => toggleRegisterPasswordVisibility("brand")}
                    />
                  </div>
                  <PasswordStrengthMeter
                    id="brand-register-password-strength"
                    password={brandRegisterForm.password}
                  />
                </div>
                ...
                <label
                  className="field-label"
                  htmlFor="brand-register-confirm-password"
                >
                  Nhập lại mật khẩu
                </label>
                <div className="input-wrapper">
                  ...
                  <input
                    ...
                    type={
                      registerPasswordVisibility.brandConfirm
                        ? "text"
                        : "password"
                    }
                    placeholder="Nhập lại mật khẩu"
                    autoComplete="new-password"
                    value={brandRegisterForm.confirmPassword}
                    onChange={(e) =>
                      setBrandRegisterForm({
                        ...brandRegisterForm,
                  ...
                  />
                  <PasswordToggleButton
                    visible={registerPasswordVisibility.brandConfirm}
                    label="mật khẩu xác nhận"
                    onToggle={() =>
                      toggleRegisterPasswordVisibility("brandConfirm")
                    }
                  />
                </div>

                <label className="field-label" htmlFor="brand-register-name">
                  Tên thương hiệu
                </label>
                <div className="input-wrapper">
                  <FieldIcon type="building" />
                  <input
                    id="brand-register-name"
                    type="text"
                    placeholder="VD: Cohan Restaurant"
                    autoComplete="organization"
                    value={brandRegisterForm.brandName}
                    onChange={(e) =>
                      setBrandRegisterForm({
                        ...brandRegisterForm,
                ...
                <label
                  className="field-label"
                  htmlFor="brand-register-restaurant"
                >
                  Chi nhánh đầu tiên
                </label>
                <div className="input-wrapper">
                  <FieldIcon type="building" />
                  <input
                    id="brand-register-restaurant"
                    type="text"
                    placeholder="Để trống sẽ dùng tên thương hiệu"
                    autoComplete="organization"
                    value={brandRegisterForm.firstRestaurantName}
                    onChange={(e) =>
                      setBrandRegisterForm({
                        ...brandRegisterForm,
                        firstRestaurantName: e.target.value,
                      })
                    }
                  />
                </div>

                <label className="field-label" htmlFor="brand-register-address">
                  Địa chỉ chi nhánh
                </label>
                <div className="input-wrapper">
                  <FieldIcon type="mapPin" />
                  <input
                    id="brand-register-address"
                    type="text"
                    placeholder="Địa chỉ"
                    autoComplete="street-address"
                    value={brandRegisterForm.address}
                    onChange={(e) =>
                      setBrandRegisterForm({
                        ...brandRegisterForm,
                ...
                <label className="check-card check-card--terms">
                  <input
                    type="checkbox"
                    checked={brandRegisterForm.terms}
                    onChange={(e) =>
                      setBrandRegisterForm({
                        ...brandRegisterForm,
              ...
              <>
                ...
                <div className="password-field">
                  <div className="input-wrapper">
                    ...
                    <input
                      ...
                      type={
                        registerPasswordVisibility.customer
                          ? "text"
                          : "password"
                      }
                      placeholder="Mật khẩu"
                    ...
                    />
                    <PasswordToggleButton
                      visible={registerPasswordVisibility.customer}
                      onToggle={() =>
                        toggleRegisterPasswordVisibility("customer")
                      }
                    />
                  </div>
                  <PasswordStrengthMeter
                ...
                <div className="input-wrapper">
                  ...
                  <input
                    ...
                    type={
                      registerPasswordVisibility.customerConfirm
                        ? "text"
                        : "password"
                    }
                    placeholder="Nhập lại mật khẩu"
                  ...
                  />
                  <PasswordToggleButton
                    visible={registerPasswordVisibility.customerConfirm}
                    label="mật khẩu xác nhận"
                    onToggle={() =>
                      toggleRegisterPasswordVisibility("customerConfirm")
                    }
                  />
                </div>
            ...
            <button
              ...
              disabled={
                submitLoading || (!isBrandRegister && captchaConfigMissing)
              }
            >
              {submitLoading
                ? "Đang tạo..."
        ...
        >
          ...
          >
            ...
            <button
              ...
              onClick={() =>
                showNotification(
                  "Vui lòng liên hệ quản trị viên nhà hàng hoặc bộ phận hỗ trợ để đặt lại mật khẩu.",
                  "info",
                )
              }
            >
        ...
        <div className="overlay-container">
          <div className="overlay">
            ...
            <div className="overlay-panel overlay-left">
              <h1>Chào mừng</h1>
              <p>
                Đăng ký khách hàng để đặt món, hoặc đăng ký thương hiệu để tạo
                thương hiệu và chi nhánh đầu tiên.
              </p>
              <div className="overlay-access-stack">
                ...
                <span>
                  <strong>Thương hiệu</strong>
                  <small>Tạo thương hiệu, chi nhánh, POS</small>
                </span>
              </div>
              <button
                type="button"
            ...
            <div className="overlay-panel overlay-right">
              <h1>Xin chào</h1>
              <p>
                Đăng nhập Cohan để tiếp tục với đúng vai trò: khách hàng hoặc
                đội vận hành thương hiệu.
              </p>
              <div className="overlay-access-stack">
                <span>
                  <strong>Khách hàng</strong>
                  <small>Theo dõi đơn & ưu đãi</small>
                </span>
                <span>
                  <strong>Thương hiệu</strong>
                  <small>Kiểm soát thương hiệu, bàn, ca, POS</small>
                </span>
              </div>
              <button
                type="button"
