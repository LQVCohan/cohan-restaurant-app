...
const FloorPlanDesigner = () => {
  ...
  const handleGenerateSmartLayout = async () => {
    ...
    const generatedDecor = (layoutPayload?.decor || []).map((decor) => ({
      ...decor,
      source: "smart_layout",
    ...
    const newLocalTables = sourceTables.slice(mappedCount).map((pos, index) => {
      ...
      return { id: `tmp_ai_${Date.now()}_${index}`, type: "table", x: Number(pos.x) || startX, y: Number(pos.y) || startY, w: 60, h: 60, rotation: Number(pos.rotation) || 0, label: generatedCode, code: generatedCode, capacity: Number(pos.capacity) || 4, tableType: pos.type || "standard", isRealTable: true, isLocalOnly: true, source: "smart_layout", isAutoGenerated: true };
    });

    pushHistory(items);
    setItems((prev) => {
      const mappedById = new Map(mappedExistingTables.map((table) => [table.id, table]));
      const manualOrPreservedItems = prev.filter((item) => {
        if (item.isRealTable) return true;
        if (item.isLocalOnly) return false;
        if (isSmartLayoutGeneratedItem(item)) return false;
        return true;
      });
      const withUpdatedTables = manualOrPreservedItems.map((item) => mappedById.get(item.id) || item);
