...
export default function TableQrManagementPage() {
  ...
  const filteredTables = useMemo(() => {
    ...
    return tables.filter((table) => {
      ...
      const matchesSearch = !keyword || String(table.code || "").toLowerCase().includes(keyword);
      const matchesFloor = floorFilter === "all" || String(table.floorLevel || 0) === floorFilter;
      const matchesQr = qrFilter === "all" || state === qrFilter;
      return matchesSearch && matchesFloor && matchesQr;
    });
