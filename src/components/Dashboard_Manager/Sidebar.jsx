...
const Sidebar = ({ isOpen, onClose, onToggle, onPageChange, activeItem }) => {
  ...
  const navigationSections = [
    ...
    { title: "Khách hàng", items: [
      ...
      { id: "ai-chatbot-knowledge", permissions: ["ai.chatbot.read", "ai.chatbot.write", "ai.chatbot.moderate", "ai.chatbot.evaluate"], icon: "📚", label: "AI Chatbot Knowledge", page: "AI Chatbot Knowledge" },
  ...
  const handleItemClick = (item) => { onPageChange(item.id); if (window.innerWidth <= 768) onClose(); };
  ...
  return (
    <>
      <aside className={`sidebar ${isOpen ? "sidebar-open" : ""}`} aria-label="Thanh điều hướng quản lý">
        <button className="sidebar-rail-toggle" onClick={onToggle} type="button" aria-label={isOpen ? "Thu gọn menu quản lý" : "Mở menu quản lý"} aria-expanded={isOpen}><span aria-hidden="true">{isOpen ? "‹" : "›"}</span></button>
          ...
          {visibleSections.map((section, sectionIndex) => (
            <div key={sectionIndex} className="nav-section">
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button key={item.id} className={`nav-item ${isAiItem ? "nav-item--ai" : ""} ${activeItem === item.id ? "active" : ""}`} onClick={() => handleItemClick(item)} title={item.label} type="button" aria-current={activeItem === item.id ? "page" : undefined} aria-label={item.label} data-tooltip={item.label}>
                    <span className="nav-icon">{item.icon}</span><span className="nav-label">{item.label}</span>{activeItem === item.id && <div className="nav-indicator" />}
                  </button>
                );
              })}
            </div>
        ...
        <div className="sidebar-footer"><div className="sidebar-user"><div className="user-avatar-small">👨‍💼</div><div className="user-info-small"><div className="user-name-small">{sidebarUserName}</div><div className="user-status-small"><span className="status-dot-small"></span>{sidebarUserRole}</div></div></div></div>
      </aside>
    </>
  );
};
