...
export default function ComboManagement() {
  ...
  const originalPrice = form.items.reduce(
    (sum, row) => sum + Number(menuById.get(String(row.menuItemId))?.basePrice || 0) * Number(row.qty || 1),
    0,
  );
  ...
  const addItem = () => {
    ...
    setForm((previous) => ({
      ...previous,
      items: [...previous.items, { menuItemId: menuItems[0].id, qty: 1 }],
    }));
  };
  ...
  const submit = async (event) => {
    ...
    if (!form.name.trim() || Number(form.price) <= 0 || !form.items.length || form.items.some((item) => !item.menuItemId)) {
    ...
    <main className="combo-management">
      <ManagementPageHeader
        ...
        stats={[
          ...
          { id: "items", label: "Tổng suất món", value: summary.itemQuantity, icon: <Utensils size={17} aria-hidden="true" /> },
          { id: "saving", label: "Tiết kiệm TB", value: money(summary.averageSaving), icon: <BadgeDollarSign size={17} aria-hidden="true" /> },
        ]}
        primaryAction={{
          label: "Tạo combo",
      ...
      {!hasRestaurants ? (
              ...
              <article className="manager-combo-card" key={combo.id} role="listitem">
                ...
                <div className="manager-combo-card__body">
                  ...
                  <footer className="manager-combo-card__footer">
                    <span className="manager-combo-card__updated"><CalendarDays size={14} aria-hidden="true" />{formatDate(combo.updatedAt || combo.createdAt)}</span>
                    <div className="manager-combo-card__actions">
                      ...
                      <button type="button" onClick={() => handleToggle(combo)} disabled={Boolean(busyKey)} aria-label={`${combo.isActive ? "Tắt" : "Bật"} ${combo.name}`}>
                        ...
                        {busyKey === `toggle:${combo.id}` ? "Đang lưu" : combo.isActive ? "Tắt" : "Bật"}
                      </button>
        ...
        <section className="combo-management__state">
          ...
          <p>Tạo combo đầu tiên hoặc đổi từ khóa và trạng thái để xem dữ liệu.</p>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editing ? "Sửa combo" : "Tạo combo"} size="lg" className="combo-management__dialog" autoWrapBody={false}>
        <form className="combo-management__form" onSubmit={submit}>
          <div className="combo-management__dialog-intro">
            ...
            <p>{menuError ? "Không thể tải danh sách món của nhà hàng." : menuLoading ? "Đang tải món..." : `${menuItems.length} món có thể thêm vào combo.`}</p>
          </div>
          <label>Tên combo<input required autoFocus value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="VD: Combo trưa no đủ" /></label>
          <label>Mô tả<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Mô tả ngắn để nhân viên và khách dễ hiểu" /></label>
          <div className="combo-management__form-grid">
            <label>Ảnh combo<input value={form.imageUrl} onChange={(event) => setForm({ ...form, imageUrl: event.target.value })} placeholder="https://..." /></label>
            <label>Giá combo<input required type="number" min="1" value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} /></label>
          </div>
          <div className="combo-management__items">
            <div className="combo-management__items-heading">
              <div><strong>Món trong combo</strong><span>{form.items.length} món đã chọn</span></div>
              <button type="button" className="combo-management__secondary-button" onClick={addItem} disabled={menuLoading || Boolean(menuError)}><Plus size={15} aria-hidden="true" />Thêm món</button>
            </div>
            {form.items.map((item, index) => (
              <div className="combo-management__item-row" key={`${item.menuItemId}-${index}`}>
                <select aria-label={`Chọn món ${index + 1}`} value={item.menuItemId} onChange={(event) => updateItem(index, { menuItemId: event.target.value })}>
                  {menuItems.map((menuItem) => <option key={menuItem.id} value={menuItem.id}>{menuItem.name} · {money(menuItem.basePrice)}</option>)}
                </select>
                <input aria-label={`Số lượng món ${index + 1}`} type="number" min="1" required value={item.qty} onChange={(event) => updateItem(index, { qty: event.target.value })} />
          ...
          <div className="combo-management__preview" aria-live="polite">
            <span>Giá món lẻ <strong>{money(originalPrice)}</strong></span>
            <span>Khách tiết kiệm <strong>{money(saving)}</strong></span>
          </div>
          <div className="combo-management__form-footer">
            <label className="combo-management__check"><input type="checkbox" checked={form.isActive} onChange={(event) => setForm({ ...form, isActive: event.target.checked })} />Mở bán combo sau khi lưu</label>
            <button type="submit" className="combo-management__submit" disabled={mutationLoading}><Save size={16} aria-hidden="true" />{mutationLoading ? "Đang lưu..." : "Lưu combo"}</button>
          </div>
        </form>
      </Modal>
    </main>
