...
const tabLabels = { knowledge: "Tri thức", suggestions: "Gợi ý", feedback: "Phản hồi", safety: "An toàn", evaluation: "Đánh giá" };
...
export default function AiChatbotKnowledgePage() {
  ...
  const renderKnowledge = () => (
    <div className="ai-admin-grid ai-admin-grid--knowledge">
      <article className="ai-admin-panel">
        ...
        {knowledge.length ? <div className="ai-admin-card-list">{knowledge.map((item) => <article key={item?.id} className={`ai-admin-card ${selectedKnowledge.includes(item?.id) ? "is-selected" : ""}`}><input className="ai-admin-card__checkbox" aria-label={`knowledge-${item?.id}`} type="checkbox" checked={selectedKnowledge.includes(item?.id)} onChange={() => toggleSelection(item?.id, selectedKnowledge, setSelectedKnowledge)} /><div className="ai-admin-card__body"><div className="ai-admin-card__meta"><span className={statusClass(item?.enabled)}>{item?.enabled ? "Đang bật" : "Đang tắt"}</span><span>{item?.category || "general"}</span><span>Priority {item?.priority ?? 0}</span><span>{item?.sourceType || "manual"}</span></div><h4>{item?.title}</h4><p>{item?.content}</p><div className="ai-admin-tag-row">{safeTags(item?.tags).map((tag) => <span key={tag} className="ai-admin-tag">{tag}</span>)}{!safeTags(item?.tags).length ? <span className="ai-admin-tag ai-admin-tag--muted">Không có tag</span> : null}</div><small>Cập nhật {formatDate(item?.updatedAt || item?.createdAt)}</small></div><div className="ai-admin-card__actions"><button type="button" onClick={() => editKnowledge(item)}>Sửa</button><button type="button" className="ai-admin-button--secondary" onClick={() => runAction(() => updateKnowledge({ variables: { input: { id: item?.id, enabled: !item?.enabled } } }), item?.enabled ? "Đã tắt item." : "Đã bật item.")}>{item?.enabled ? "Tắt" : "Bật"}</button><button type="button" className="ai-admin-button--danger" onClick={() => window.confirm("Xóa knowledge item này?") && runAction(() => deleteKnowledge({ variables: { id: item?.id } }), "Đã xóa knowledge item.")}>Xóa</button></div></article>)}</div> : <EmptyState title="Chưa có knowledge item" description="Thêm nội dung thủ công hoặc import JSON/CSV để chatbot có nguồn trả lời rõ ràng hơn." />}
      </article>
  ...
  return <section className="ai-admin-page ai-admin-page--knowledge">
    ...
    <nav className="ai-admin-tabs" aria-label="AI chatbot knowledge tabs">{tabs.map((tab) => <button key={tab} type="button" className={activeTab === tab ? "is-active" : ""} onClick={() => setActiveTab(tab)}>{tabLabels[tab] || tab}</button>)}</nav>
