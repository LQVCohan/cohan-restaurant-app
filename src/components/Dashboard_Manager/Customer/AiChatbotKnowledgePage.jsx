...
export default function AiChatbotKnowledgePage() {
  ...
  return <section style={{ padding: 16 }}>
    ...
    <ul>{suggestionRows.map((s) => <li key={s.id}>
      ...
      {s.status === "pending" ? <>
        <button onClick={async () => { const title = window.prompt("Title", s.suggestedTitle || s.question) || s.question; const content = window.prompt("Content", s.suggestedContent || s.question) || s.question; await approveSuggestion({ variables: { id: s.id, input: { title, content, category: s.category || "", tags: s.tags || [], sourceType: "faq", enabled: true, priority: 50 } } }); refetchSuggestion?.(); refetch?.(); }}>Approve</button>
