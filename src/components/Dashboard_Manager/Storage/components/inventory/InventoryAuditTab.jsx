...
function InventoryAuditTab({
  ...
  const canOpenCountStep = Boolean(activeCount);
  ...
  if (error) {
    return (
      <div className="inv-error-state" role="alert">
        ...
        {onReload && (
          <button type="button" className="inv-primary-btn" onClick={onReload}>Thử tải lại</button>
        )}
      </div>
    );
  }
    ...
    <div className="inventory-audit-tab">
      ...
      <section className="inv-overview-grid" aria-label="Tình trạng tồn kho">
        <article className="inv-overview-card">
          <span className="inv-overview-card__icon" aria-hidden="true"><Boxes size={17} /></span>
          <div><span>Đang theo dõi</span><strong>{summary.total}</strong><small>mặt hàng</small></div>
        </article>
        <article className="inv-overview-card inv-overview-card--ok">
          <span className="inv-overview-card__icon" aria-hidden="true"><CheckCircle2 size={17} /></span>
          <div><span>Ổn định</span><strong>{summary.ok}</strong><small>trên định mức</small></div>
        </article>
        <article className="inv-overview-card inv-overview-card--warn">
          <span className="inv-overview-card__icon" aria-hidden="true"><AlertCircle size={17} /></span>
          <div><span>Sắp hết</span><strong>{summary.low}</strong><small>cần nhập thêm</small></div>
        </article>
        <article className="inv-overview-card inv-overview-card--danger">
          <span className="inv-overview-card__icon" aria-hidden="true"><AlertCircle size={17} /></span>
          <div><span>Hết hàng</span><strong>{summary.out}</strong><small>cần xử lý</small></div>
        </article>
      </section>
      ...
      {!canUseCount && (
        <div className="inv-scope-warning" role="status">
          <AlertCircle size={18} />
          <div>
            <strong>Vui lòng chọn một kho cụ thể để bắt đầu kiểm kê.</strong>
            <span>Dữ liệu tổng hợp vẫn hiển thị, nhưng chưa thể tạo kỳ kiểm kê.</span>
          </div>
        </div>
      )}
      ...
      <nav className="inv-stepper" aria-label="Quy trình kiểm kê">
        <button
          type="button"
          className={activeStep === "setup" ? "is-active" : ""}
          aria-current={activeStep === "setup" ? "step" : undefined}
          onClick={() => openStep("setup")}
        >
          <span>1</span><strong>Tạo kỳ</strong><small>Chọn phạm vi kiểm</small>
        </button>
        <button
          type="button"
          className={activeStep === "count" ? "is-active" : ""}
          aria-current={activeStep === "count" ? "step" : undefined}
          disabled={!canOpenCountStep}
          onClick={() => openStep("count")}
        >
          <span>2</span><strong>Kiểm đếm</strong><small>Nhập số thực tế</small>
        </button>
        <button
          type="button"
          className={activeStep === "documents" ? "is-active" : ""}
          aria-current={activeStep === "documents" ? "step" : undefined}
          disabled={!canOpenDocumentStep}
          onClick={() => openStep("documents")}
        >
          <span>3</span><strong>Đối chiếu</strong><small>Chứng từ và biến động</small>
        </button>
      </nav>
      ...
      {activeStep === "setup" && (
        <section className="inv-workspace-panel" aria-labelledby="inventory-setup-title">
          <div className="inv-section-heading">
            <div>
              <p className="inv-section-kicker">Bước 1</p>
              <h3 id="inventory-setup-title"><ClipboardCheck size={19} /> Kỳ kiểm kê</h3>
              <p>Tạo kỳ mới hoặc chọn một kỳ đã có để tiếp tục.</p>
            </div>
          </div>
          ...
          <div className="inv-count-layout">
            ...
            >
              <div className="inv-card-heading">
                <div><span>Tạo kỳ mới</span><strong>Chụp tồn hệ thống tại thời điểm bắt đầu</strong></div>
              </div>
              ...
              >
                {creatingCount ? "Đang tạo kỳ…" : "Tạo kỳ kiểm kê"}
              </button>
            </form>

            <div className="inv-active-count">
              <div className="inv-card-heading">
                <div><span>Kỳ hiện có</span><strong>{activeCount?.title || "Chưa có kỳ kiểm kê"}</strong></div>
              </div>
              ...
              {activeCount ? (
                <div className="inv-active-count__body">
                  ...
                  <div className="inv-count-meta-grid">
                    ...
                    <div><span>Đã đếm</span><strong>{countSummary.counted}/{countSummary.total}</strong></div>
                    <div><span>Chênh lệch</span><strong>{countSummary.varianceLines} dòng</strong></div>
                    <div><span>Trạng thái</span><strong>{countStatusLabel[activeCount.status]}</strong></div>
                  </div>
                  <button type="button" className="inv-primary-btn inv-primary-btn--wide" onClick={() => openStep("count")}>
                    Tiếp tục kiểm đếm
                  </button>
                </div>
              ) : (
                <div className="inv-empty-state inv-empty-state--compact">
                  ...
                  <span>Tạo kỳ đầu tiên để mở bước kiểm đếm.</span>
                </div>
              )}
            </div>
          </div>
      ...
      {activeStep === "count" && activeCount && (
        <section className="inv-workspace-panel" aria-labelledby="inventory-count-title">
          <div className="inv-section-heading">
            <div>
              <p className="inv-section-kicker">Bước 2</p>
              <h3 id="inventory-count-title"><Boxes size={19} /> Kiểm đếm thực tế</h3>
              <p>Nhập số lượng thực tế, kiểm tra tồn khả dụng và chốt kỳ khi hoàn tất.</p>
            </div>
            <div className="inv-progress-block inv-progress-block--heading">
              <div><span>Tiến độ</span><strong>{completionPercent}%</strong></div>
              <progress value={countSummary.counted} max={Math.max(countSummary.total, 1)}>{completionPercent}%</progress>
            </div>
          </div>

          <div className="inv-count-summary-line">
            <span><strong>{activeCount.code}</strong> · {countStatusLabel[activeCount.status]}</span>
            <span>{countSummary.counted}/{countSummary.total} dòng đã đếm</span>
            <span className={countSummary.netVariance === 0 ? "" : countSummary.netVariance > 0 ? "inv-positive" : "inv-negative"}>
              Lệch ròng: <strong>{formatQty(countSummary.netVariance)}</strong>
            </span>
          </div>
          ...
          <div className="inv-count-lines">
            <div className="inv-table-shell inv-count-table-shell">
              <table className="inv-table inv-count-table">
                <thead>
                  <tr>
                    <th>Nguyên liệu</th><th>SKU</th><th>Hệ thống</th><th>Thực tế</th><th>Chênh lệch</th><th>Ghi chú</th><th>Thao tác</th>
                  </tr>
                </thead>
                <tbody>
                  {(activeCount.lines || []).slice(0, 80).map((line) => {
                    ...
                    return (
                      <tr key={line.ingredientId}>
                        ...
                        <td data-label="Chênh lệch" className={variance === 0 ? "" : variance > 0 ? "inv-positive" : "inv-negative"}>
          ...
          <div className="inv-stock-subsection">
            <div className="inv-subsection-heading">
              <div><span>Tham chiếu</span><h4>Tồn khả dụng hiện tại</h4></div>
              <small>Đối chiếu nhanh với định mức trước khi chốt kỳ.</small>
            </div>
            ...
            <div className="inv-toolbar">
              ...
              <div className="inv-filter-group" role="group" aria-label="Lọc trạng thái tồn kho">
                {[
                  ...
                  <button
                    ...
                    onClick={() => { setPage(1); setStockFilter(value); }}
                  >
              ...
              <button
                ...
                onClick={() => setSortBy((current) => current === "name" ? "available" : current === "available" ? "status" : "name")}
              >
            ...
            <div className="inv-table-shell">
              <table className="inv-table inv-stock-table">
                <thead><tr><th>Nguyên liệu</th><th>SKU</th><th>Tồn khả dụng</th><th>Định mức</th><th>Số kho</th><th>Trạng thái</th></tr></thead>
                <tbody>
                  {pagedRows.map((item) => (
                    <tr key={item.ingredientId} className={`inv-stock-row inv-stock-row--${item.status}`}>
                      ...
                      <td data-label="Trạng thái"><span className={`inv-badge inv-badge--${item.status}`}>{item.status === "ok" ? "Ổn định" : item.status === "low" ? "Sắp hết" : "Hết hàng"}</span></td>
                    </tr>
                  ))}
                  {!pagedRows.length && (
                    <tr><td colSpan={6} className="inv-empty-row"><div className="inv-empty-state inv-empty-state--compact"><Search size={24} /><strong>Không có mặt hàng phù hợp</strong><span>Đổi từ khóa hoặc xóa bộ lọc.</span></div></td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="inv-step-actions">
            <button type="button" className="inv-secondary-btn" onClick={() => openStep("setup")}>Quay lại</button>
            {activeCount.status === "draft" ? (
              ...
              >
                {closingCount ? "Đang chốt kỳ…" : "Chốt kỳ và sang đối chiếu"}
              </button>
            ) : (
              <button type="button" className="inv-primary-btn" onClick={() => openStep("documents")}>Sang đối chiếu</button>
            )}
          </div>
        </section>
      )}
      ...
      {activeStep === "documents" && canOpenDocumentStep && (
        <section className="inv-workspace-panel" aria-labelledby="inventory-document-title">
          <div className="inv-section-heading">
            <div>
              <p className="inv-section-kicker">Bước 3</p>
              <h3 id="inventory-document-title"><FileCheck2 size={19} /> Đối chiếu chứng từ</h3>
              <p>Gắn số phiếu, xác nhận tình trạng chứng từ và kiểm tra biến động gần nhất.</p>
            </div>
          </div>
                    ...
                    <article className={`inv-document-item inv-document-item--${documentStatusTone[draft.status] || "pending"}`} key={movement.id}>
                      <div className="inv-document-item__head">
                        <div>
                          ...
                          <span>{movementLabel[movement.type] || movement.type} · {formatQty(movement.qty)} {ingredient?.baseUnit || ""}</span>
                          <small>{formatDateTime(movement.createdAt)} · {movement.reason || "Không có lý do"}</small>
                        </div>
                        <span className={`inv-doc-status inv-doc-status--${documentStatusTone[draft.status] || "pending"}`}>{documentStatusLabel[draft.status] || draft.status}</span>
                      </div>
                      <div className="inv-document-fields">
                        <label>Số phiếu<input name={`doc-no-${movement.id}`} autoComplete="off" value={draft.documentNo} onChange={(event) => setDocumentDraft(movement, { documentNo: event.target.value })} placeholder="Ví dụ: PN-0001" /></label>
                        <label>Trạng thái<select name={`doc-status-${movement.id}`} value={draft.status} onChange={(event) => setDocumentDraft(movement, { status: event.target.value })}>{Object.entries(documentStatusLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
                        <label className="inv-document-note">Ghi chú<input name={`doc-note-${movement.id}`} autoComplete="off" value={draft.note} onChange={(event) => setDocumentDraft(movement, { note: event.target.value })} placeholder="Nội dung lệch hoặc thiếu" /></label>
                        <button type="button" className="inv-small-btn" onClick={() => handleSaveDocument(movement)} disabled={reconcilingDocument}>Lưu đối chiếu</button>
                      </div>
                    </article>
                  );
                })}
                {!documentMovements.length && <div className="inv-empty-state inv-empty-state--compact"><FileCheck2 size={24} /><strong>Chưa có biến động cần đối chiếu</strong><span>Phiếu mới sẽ xuất hiện tại đây.</span></div>}
              </div>
            </div>
          ...
          <div className="inv-step-actions"><button type="button" className="inv-secondary-btn" onClick={() => openStep("count")}>Quay lại kiểm đếm</button></div>
        </section>
      )}
    </div>
  );
