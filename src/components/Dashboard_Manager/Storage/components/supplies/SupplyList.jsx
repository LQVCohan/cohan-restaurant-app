...
const SupplyList = ({ restaurantId, warehouseId = null, warehouses = [], warehousesLoading = false, onRegisterActions }) => {
  ...
  const canTransfer = useMemo(
    () => warehouses.filter((warehouse) => warehouse?.isActive !== false).length > 1,
    [warehouses],
  );
  ...
  const openTransfer = (s) => {
    if (!canTransfer) {
      showNotification("Nhà hàng cần ít nhất hai kho hoạt động để điều chuyển vật tư.", "warning");
      return;
    }
    setCurrent(s);
    setMode("transfer");
  ...
  const submitTransfer = async (values) => {
    ...
    catch (err) { showNotification(getSupplyActionErrorMessage(err, toFriendlyTransferError(err)), "error"); throw err; }
  };
  ...
  return (
    <div className="supply-list-container">
      ...
      {mode === "transfer" && current && canTransfer && <StockTransferModal isOpen onClose={closeStockModal} onConfirm={submitTransfer} supply={current} warehouses={warehouses || []} />}
    </div>
  );
};
...
function toFriendlyTransferError(error) { const graphQLErrors = error?.graphQLErrors || error?.networkError?.result?.errors || []; const message = graphQLErrors[0]?.message || error?.message || ""; const code = graphQLErrors[0]?.extensions?.code || ""; const currentAvailable = graphQLErrors[0]?.extensions?.currentAvailable; if (code === "STOCK_ITEM_NOT_FOUND") return "Vật tư chưa có tồn kho tại kho nguồn."; if (code === "INSUFFICIENT_STOCK") { if (Number.isFinite(Number(currentAvailable))) return `Không đủ tồn khả dụng tại kho nguồn. Có thể chuyển: ${Number(currentAvailable).toLocaleString("vi-VN")}.`; return "Không đủ tồn khả dụng tại kho nguồn."; } if (code === "BAD_USER_INPUT") return "Thông tin điều chuyển chưa hợp lệ. Vui lòng kiểm tra lại kho nguồn và kho đích."; return message.replace(/^GraphQL error:\s*/i, "").trim() || "Không thể chuyển kho vật tư."; }
