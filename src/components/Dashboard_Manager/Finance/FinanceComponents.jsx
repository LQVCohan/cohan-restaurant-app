...
export const ReceivableDebts = ({ debts = [] }) => (
  <div className="debt-list">
    {debts.length === 0 ? (
      <div className="p-3 text-muted text-center">Không có hóa đơn chưa thanh toán / thanh toán một phần</div>
    ) : (
      debts.map((d) => (
        <div key={d.id} className="debt-item">
          <div className="debt-info">
            <div className="supplier-name">{d.supplier}</div>
            <div className="due-date text-danger">Cập nhật: {d.dueDate ? new Date(d.dueDate).toLocaleDateString("vi-VN") : "N/A"}</div>
          </div>
          ...
          transactions.map((t) => {
            ...
            return (
              <tr key={t.id} onClick={() => onSelect?.(t)} className={onSelect ? "clickable-row" : ""}>
                ...
                <td><span className={`badge ${t.status === "completed" || t.status === "SUCCESS" ? "success" : "warning"}`}>{t.status || "completed"}</span></td>
              </tr>
            );
          })
        )}
