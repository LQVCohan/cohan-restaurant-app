...
vi.mock("@/hooks/useTransactions", () => ({
  CASHFLOW_CATEGORIES: ["sale", "refund", "payroll", "inventory", "operations", "supplier_payment", "adjustment", "other"],
  CASHFLOW_SUBCATEGORIES: ["labor", "cogs", "rent", "utility", "maintenance", "marketing", "bank_fee", "tax", "etc", "other"],
  PAYMENT_METHODS: ["cash", "card", "bank_transfer", "e_wallet", "transfer", "provider", "other"],
...
function buildTx(overrides = {}) {
  return {
    ...
    restaurantId: "r1",
    filters: { dateFrom: "2026-06-01", dateTo: "2026-06-30", type: "all", category: "", subcategory: "", method: "", status: "", source: "", referenceId: "", search: "" },
    reconciliationStatus: "all",
    bankStatus: "",
    loading: false,
    error: null,
    transactions: [{ id: "trx-1", type: "INFLOW", amount: 500000, description: "Thanh toán hóa đơn", status: "completed", source: "order", category: "sale", referenceType: "PaymentTransaction", referenceId: "pay-1", occurredAt: "2026-06-02T00:00:00Z" }],
    cashflows: [],
    refunds: [
...
function renderPage({ userPermissions = ["finance.write", "finance.export", "refund.write", "reconciliation.write"], roleName = "accountant", txOverrides = {} } = {}) {
  mockState.value = buildTx(txOverrides);
  return render(
    <AuthContext.Provider value={{ user: { id: "u1", roleName, permissions: userPermissions.map((code) => ({ code })) } }}>
...
describe("TransactionManagement UC18", () => {
  ...
  it("renders UC18 tabs", () => {
    renderPage();
    ["Nhật ký", "Cashflow", "Hoàn tiền", "Đối soát", "Bank transactions", "Công nợ"].forEach((label) => {
  ...
  it("supports supplier payable create, edit, payment and void with reason modal", async () => {
    ...
    fireEvent.change(screen.getByLabelText("supplierName"), { target: { value: "Nhà cung cấp Hải Sản" } });
    fireEvent.change(screen.getByLabelText("amount"), { target: { value: "1200000" } });
    fireEvent.click(screen.getAllByRole("button", { name: "Tạo khoản phải trả" }).at(-1));
    await waitFor(() => expect(txSpies.createSupplierPayable).toHaveBeenCalledWith(expect.objectContaining({ supplierName: "Nhà cung cấp Hải Sản", amount: 1200000 })));
    ...
    fireEvent.change(screen.getByLabelText("amount"), { target: { value: "950000" } });
    fireEvent.click(screen.getByRole("button", { name: "Lưu khoản phải trả" }));
    await waitFor(() => expect(txSpies.updateSupplierPayable).toHaveBeenCalledWith("sp-1", expect.objectContaining({ amount: 950000 })));

    fireEvent.click(screen.getByRole("button", { name: "Công nợ" }));
    fireEvent.click(within(payableCard).getByRole("button", { name: "Thanh toán" }));
    fireEvent.change(screen.getByLabelText("paymentAmount"), { target: { value: "300000" } });
    fireEvent.change(screen.getByLabelText("Ghi chú bắt buộc"), { target: { value: "Thanh toán một phần" } });
    fireEvent.click(screen.getByRole("button", { name: "Ghi nhận thanh toán" }));
    await waitFor(() => expect(txSpies.recordSupplierPayment).toHaveBeenCalledWith("sp-1", expect.objectContaining({ amount: 300000, note: "Thanh toán một phần" })));
  ...
  it("hides write actions without finance/refund/reconciliation permissions", () => {
    ...
    expect(screen.queryByRole("button", { name: /Thu\/chi thủ công/i })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /Refund từ giao dịch/i })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Bank transactions" }));
    expect(screen.queryByRole("button", { name: "Auto match" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Công nợ" }));
    expect(screen.queryByRole("button", { name: "Tạo khoản phải trả" })).not.toBeInTheDocument();
  });
});
