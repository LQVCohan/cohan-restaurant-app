...
import { DASHBOARD_RESTAURANT_CHANGED_EVENT, readDashboardRestaurantId } from "@/utils/staffSyncEvents";
...
const DashboardSynchronized = () => {
  ...
  const fallbackRestaurantId = useMemo(() => (Array.isArray(restaurants) ? restaurants : []).map(getRestaurantId).find(Boolean) || "", [restaurants]);
  const [restaurantId, setRestaurantId] = useState(() => readDashboardRestaurantId() || fallbackRestaurantId);

  useEffect(() => {
    if (!restaurantId && fallbackRestaurantId) setRestaurantId(fallbackRestaurantId);
  ...
  useEffect(() => {
    ...
    window.addEventListener(DASHBOARD_RESTAURANT_CHANGED_EVENT, handleRestaurantChanged);
    return () => window.removeEventListener(DASHBOARD_RESTAURANT_CHANGED_EVENT, handleRestaurantChanged);
  ...
  const openStaffPage = () => {
    ...
    if (window.location.hash !== "#staff") window.location.hash = "staff";
  };

  return (
    <div ref={shellRef} className="dashboard-synchronized-shell">
      <Dashboard />
      {portalTarget && restaurantId ? createPortal(<DashboardStaffRoster restaurantId={restaurantId} onOpenStaff={openStaffPage} />, portalTarget) : null}
    </div>
  );
};
