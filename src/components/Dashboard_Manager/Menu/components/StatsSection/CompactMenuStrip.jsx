...
const SLOT_CONFIG = {
  breakfast: { label: "Sáng", color: "#d97706", bg: "#fffbeb", border: "#fde68a" },
  lunch: { label: "Trưa", color: "#059669", bg: "#ecfdf5", border: "#a7f3d0" },
  dinner: { label: "Tối", color: "#b89365", bg: "#fdf8f3", border: "#f1e5d5" },
  late_night: { label: "Khuya", color: "#db2777", bg: "#fdf2f8", border: "#fbcfe8" },
};
...
const copyName = (source, menus) => {
  ...
  const names = new Set((menus || []).map((menu) => String(menu?.name || "").trim().toLowerCase()));
  let name = base;
  let i = 2;
...
const CompactMenuStrip = ({
  ...
  const [copyMenuMutation] = useMutation(COPY_MENU_MUTATION, {
    update(cache, { data }) {
      ...
      cache.updateQuery({ query: MENUS_QUERY, variables: { restaurantId: copied.restaurantId } }, (prev) => {
  ...
  const [deleteMenuMutation] = useMutation(DELETE_MENU_MUTATION, {
    update(cache, _result, { variables }) {
      ...
      cache.updateQuery({ query: MENUS_QUERY, variables: { restaurantId: target.restaurantId } }, (prev) => ({
        menus: (prev?.menus || []).filter((menu) => String(menu.id) !== String(id)),
      }));
  ...
  const canViewHistory = canAccessMenuManagementAction(auth?.user, MENU_MANAGEMENT_ACTIONS.VIEW);
      ...
      <div className={`cms-container ${isCollapsed ? "is-collapsed" : ""}`}>
        <div className="cms-header">
          <div className="cms-info">
            <div className="cms-icon-box"><FiLayers size={22} /></div>
            <div className="cms-title-box">
              <h3>Quản Lý Thực Đơn</h3>
              {!isCollapsed && <p>Đã tạo <strong>{menus.length}</strong> thực đơn theo khung giờ và nhóm thực đơn</p>}
            </div>
          </div>
            ...
            {!isCollapsed && (
              <>
                <div className="cms-nav-group">
                  <button className="cms-nav-btn" onClick={() => scroll("left")} disabled={!menus.length}><FiChevronLeft /></button>
                  <button className="cms-nav-btn" onClick={() => scroll("right")} disabled={!menus.length}><FiChevronRight /></button>
                </div>
                {canAddMenu && <button className="cms-btn-add" onClick={() => onAddMenu()}><FiPlus /> <span className="text">Tạo thực đơn</span></button>}
              </>
            )}
            <button className="cms-btn-toggle" onClick={() => onToggleCollapse?.()} title={isCollapsed ? "Mở rộng" : "Thu gọn"}>
              {isCollapsed ? <FiChevronDown size={20} /> : <FiChevronUp size={20} />}
            </button>
          </div>
        </div>

        {!isCollapsed && menusError && <div className="cms-error-msg">Lỗi: {menusError.message}</div>}
        {!isCollapsed && actionError && <div className="cms-error-msg">Lỗi: {actionError}</div>}
          ...
          <div className="cms-viewport">
            <div className="cms-track" ref={scrollRef}>
              {!menusLoading && menus.map((menu) => {
                ...
                return (
                  <div key={menu.id} className={`cms-card ${active ? "cms-active" : ""} ${menu.isActive === false ? "cms-disabled" : ""}`} onClick={() => selectMenu(menu)}>
                    {active && <div className="cms-indicator" />}
                    <div className="cms-card-top">
                      <div className="cms-img-box">
                        {menu.coverImage ? <LocalImageView src={menu.coverImage} alt={menu.name} variant={LOCAL_IMAGE_VARIANTS.THUMB} fallback={<div className="cms-placeholder">🍽️</div>} /> : <div className="cms-placeholder">🍽️</div>}
                        {menu.categoryMenu?.name && <span className="cms-cate-badge">Nhóm thực đơn: {menu.categoryMenu.name}</span>}
                      </div>
                      <div className="cms-badges">
                        <span className="cms-slot-tag" style={{ color: slot.color, background: slot.bg, borderColor: slot.border }}><FiClock size={10} style={{ marginRight: 4 }} /> {slot.label}</span>
                        {menu.isActive === false && <span className="cms-status-off">Đang ẩn</span>}
                      </div>
                    </div>
                    ...
                    <div className="cms-toolbar">
                      {canEditMenu && <><button className="cms-tool-btn is-edit" title="Chỉnh sửa thực đơn" onClick={(e) => { e.stopPropagation(); onEditMenu(menu); }}><FiEdit3 /> <span>Sửa</span></button><div className="cms-div" /></>}
                      {canToggleMenuActive && <button className={`cms-tool-btn ${menu.isActive === false ? "is-show" : "is-hide"}`} title={menu.isActive === false ? "Bật lại thực đơn" : "Ẩn thực đơn"} onClick={(e) => { e.stopPropagation(); onToggleMenuActive(menu); }}>{menu.isActive === false ? <FiEye /> : <FiEyeOff />}<span>{menu.isActive === false ? "Bật" : "Ẩn"}</span></button>}
                      {canCopyMenu && <button className="cms-tool-btn" title="Sao chép thực đơn kèm món và recipe" disabled={!!busyMenuId} onClick={(e) => { e.stopPropagation(); handleCopyMenu(menu); }}><FiCopy />{busy && <span>...</span>}</button>}
                      {canViewHistory && <button className="cms-tool-btn is-history" title="Xem lịch sử thay đổi" onClick={(e) => { e.stopPropagation(); setHistoryMenu(menu); }}><FiActivity /></button>}
                      {canDeleteMenu && <button className="cms-tool-btn is-delete" title="Xóa thực đơn" disabled={!!busyMenuId} onClick={(e) => { e.stopPropagation(); handleDeleteMenu(menu); }}><FiTrash2 />{busy && <span>...</span>}</button>}
                    </div>
                  </div>
                );
              })}
              {!menusLoading && canAddMenu && <div className="cms-card cms-ghost" onClick={() => onAddMenu()}><div className="cms-ghost-inner"><div className="cms-ghost-circle"><FiPlus size={24} /></div><h4>Thêm thực đơn</h4></div></div>}
              <div className="cms-spacer" />
            </div>
          </div>
        )}
      </div>
      <AuditLogModal isOpen={!!historyMenu} onClose={() => setHistoryMenu(null)} restaurantId={historyMenu?.restaurantId} entity="Menu" entityId={historyMenu?.id || historyMenu?._id} title={`Lịch sử menu: ${historyMenu?.name || "Thực đơn"}`} />
    </>
  );
};
