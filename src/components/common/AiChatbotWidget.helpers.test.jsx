...
describe("AiChatbotWidget helpers", () => {
  ...
  it("feature map matches natural-language query aliases by role", () => {
    ...
    expect(getAiChatbotFeatureMatches({ pathname: "/", restaurantId: "resto-1", userRole: "customer", query: "xem thực đơn nhà hàng" }))
      .toEqual(expect.arrayContaining([expect.objectContaining({ key: "restaurant-menu", path: "/restaurant/resto-1#menu" })]));
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "xem thực đơn nhà hàng" }).some((entry) => entry.key === "restaurant-menu"))
      .toBe(false);
    ...
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "xem đánh giá nhà hàng" }).some((entry) => entry.key === "restaurant-reviews"))
      .toBe(false);
    expect(getAiChatbotFeatureMatches({ pathname: "/", restaurantId: "resto-1", userRole: "customer", query: "ưu đãi nhà hàng" }))
      .toEqual(expect.arrayContaining([expect.objectContaining({ key: "restaurant-promotions", path: "/restaurant/resto-1#promotions" })]));
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "ưu đãi nhà hàng" }).some((entry) => entry.key === "restaurant-promotions"))
      .toBe(false);
    expect(getAiChatbotFeatureMatches({ pathname: "/", restaurantId: "resto-1", userRole: "customer", query: "xem ảnh nhà hàng" }))
      .toEqual(expect.arrayContaining([expect.objectContaining({ key: "restaurant-photos", path: "/restaurant/resto-1#photos" })]));
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "xem ảnh nhà hàng" }).some((entry) => entry.key === "restaurant-photos"))
      .toBe(false);
    ...
    expect(getAiChatbotFeatureMatches({ pathname: "/track-delivery/order-1", userRole: "customer", query: "theo dõi giao hàng" }))
      .toEqual(expect.arrayContaining([expect.objectContaining({ key: "order-tracking", path: "/track-delivery/order-1" })]));
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "theo dõi giao hàng" }).some((entry) => entry.key === "order-tracking"))
      .toBe(false);
    ...
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "mục yêu thích ở đâu" }))
      .toEqual(expect.arrayContaining([expect.objectContaining({ key: "favorites", path: "/favorites" })]));
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "sổ địa chỉ của tôi" }))
      .toEqual(expect.arrayContaining([expect.objectContaining({ key: "address-book", path: "/address-book" })]));
    expect(getAiChatbotFeatureMatches({ pathname: "/", userRole: "customer", query: "trợ giúp tài khoản ở đâu" }))
      .toEqual(expect.arrayContaining([expect.objectContaining({ key: "help-center", path: "/help-center" })]));
