...
const resolveSenderLabel = (msg) => {
  ...
  if (["staff", "manager", "admin"].includes(role))
    return msg?.senderName || "Nhân viên";
...
export default function AiHandoffInbox({
  ...
  const threadItems = useMemo(
    () =>
      (activeThreads || []).filter(isAiHandoffThread).map((t) => ({
        kind: "thread",
        id: `thread_${t.id}`,
        notificationId: null,
        threadId: t.id,
  ...
  if (!canViewHandoff)
    return (
      <div className="ai-handoff-inbox__panel">
        <div className="ai-handoff-inbox__content">
          Bạn không có quyền xử lý handoff AI chatbot.
        </div>
      </div>
    );
            ...
            >
              ...
              {restaurants.map((restaurant) => (
                <option key={restaurant.id} value={restaurant.id}>
                  {restaurant.name ||
                    `Nhà hàng ${String(restaurant.id).slice(-6)}`}
                </option>
              ))}
            </select>
          </label>
