...
export const isSystemAdmin = (user) => roleName(user) === "admin";
...
export async function getScopedRestaurantFilter(user) {
  ...
  const brandIds = memberships.filter((m) => ["owner", "admin", "manager"].includes(m.role)).map((m) => m.brandId);
  const explicit = memberships.flatMap((m) => m.restaurantIds || []);
  return { $or: [{ managerId: uid }, ...(brandIds.length ? [{ brandId: { $in: brandIds } }] : []), ...(explicit.length ? [{ _id: { $in: explicit } }] : [])] };
}
...
const Mutation = {
  ...
  addBrandMember: async (_, { input }, ctx) => { if (!ctx?.user || !await canManageBrand(ctx.user, input.brandId)) throw forbidden(); if (!await User.exists({ _id: oid(input.userId) })) throw bad("User not found"); const restaurants = await ensureBrandRestaurants(input.brandId, input.restaurantIds); return BrandMembership.findOneAndUpdate({ brandId: oid(input.brandId), userId: oid(input.userId) }, { $set: { role: input.role, restaurantIds: restaurants, status: "active", updatedBy: oid(userId(ctx.user)) }, $setOnInsert: { createdBy: oid(userId(ctx.user)), invitedBy: oid(userId(ctx.user)) } }, { new: true, upsert: true }).lean(); },
  updateBrandMember: async (_, { input }, ctx) => { const m = await BrandMembership.findById(input.id); if (!m) throw bad("Membership not found"); if (!ctx?.user || !await canManageBrand(ctx.user, m.brandId)) throw forbidden(); if (m.role === "owner" && input.role && input.role !== "owner" && await BrandMembership.countDocuments({ brandId: m.brandId, role: "owner", status: "active" }) <= 1) throw bad("Cannot remove the last owner"); if (input.restaurantIds) m.restaurantIds = await ensureBrandRestaurants(m.brandId, input.restaurantIds); if (input.role) m.role = input.role; if (input.status) m.status = input.status; m.updatedBy = oid(userId(ctx.user)); await m.save(); return m.toObject(); },
  removeBrandMember: async (_, { id }, ctx) => { const m = await BrandMembership.findById(id); if (!m) throw bad("Membership not found"); if (!ctx?.user || !await canManageBrand(ctx.user, m.brandId)) throw forbidden(); if (m.role === "owner" && await BrandMembership.countDocuments({ brandId: m.brandId, role: "owner", status: "active" }) <= 1) throw bad("Cannot remove the last owner"); m.status = "inactive"; await m.save(); return true; },
};

export default { Query, Mutation, Brand: { id: (p) => p.id ?? String(p._id), owner: (p) => User.findById(p.ownerId).lean(), restaurantCount: (p) => Restaurant.countDocuments({ brandId: p._id || p.id }), restaurants: (p, { limit = 50 }) => Restaurant.find({ brandId: p._id || p.id }).limit(Math.min(limit, 100)).lean() }, BrandMembership: { id: (p) => p.id ?? String(p._id), brand: (p) => Brand.findById(p.brandId).lean(), user: (p) => User.findById(p.userId).lean(), restaurants: (p) => Restaurant.find({ _id: { $in: p.restaurantIds || [] } }).lean() } };
