...
function buildFilter(filter = {}) {
  ...
  if (filter.restaurantId) {
    if (!isOid(filter.restaurantId)) throw new GraphQLError("Invalid restaurantId");
...
async function ensureRbacAuditAccess(ctx, filter = {}) {
...
export default {
  Query: {
    auditLogs: async (_, { filter = {}, limit = 50, offset = 0 }, ctx) => {
      ...
      else requireRoles(ctx, ["ADMIN"]);
      const query = buildFilter(filter);
      const safeLimit = normalizeLimit(limit);
      const safeOffset = normalizeOffset(offset);
      const [items, total] = await Promise.all([
        AuditLog.find(query).sort({ createdAt: -1 }).skip(safeOffset).limit(safeLimit).lean({ virtuals: true }),
    ...
    rbacAuditLogs: async (_, { filter = {}, limit = 50, offset = 0 }, ctx) => {
      await ensureRbacAuditAccess(ctx, filter);
      if (filter?.restaurantId) await requireRestaurantAccess(ctx, filter.restaurantId);
