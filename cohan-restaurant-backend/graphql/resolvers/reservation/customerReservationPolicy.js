...
const isStaffLike = (ctx) => {
  const role = String(
    ctx?.user?.userType || ctx?.user?.roleName || ctx?.user?.role || "",
  ).toLowerCase();
  return (
    role.includes("staff") ||
...
const endAt = (timeTo, durationMinutes, isUnlimitedTime) => {
  ...
  return Number.isFinite(start)
    ? start + Math.max(30, Number(durationMinutes || 60)) * 60000
    : null;
};
...
async function assertNoCustomerOverlap({
  userId,
  restaurantId,
  timeTo,
  durationMinutes,
  ...
  const conflict = candidates.find((item) => {
    ...
    return overlaps(
      ...
      otherStart,
      endAt(item.timeTo, item.durationMinutes, item.isUnlimitedTime),
    );
  });
  ...
  if (conflict) {
    throw new GraphQLError(
      ...
      {
        extensions: {
          code: "USER_RESERVATION_TIME_CONFLICT",
          conflictingReservationId: conflict._id,
        },
      },
    );
  }
...
async function normalizeFreeDeposit(reservation) {
  if (!reservation || Number(reservation.depositAmount || 0) > 0) {
    return reservation;
  }
  ...
  reservation.paymentReference =
    reservation.paymentReference || "NO_DEPOSIT_REQUIRED";
...
export function withCustomerReservationPolicy(mutations = {}) {
  return {
    ...
    async createReservation(parent, { input }, ctx, info) {
      ...
      const reservation = await mutations.createReservation(
        parent,
        { input },
        ctx,
        info,
      ...
      if (!isStaffLike(ctx) && input?.type === "time") {
        ...
        if (current) {
          await assertNoCustomerOverlap({
            ...
            durationMinutes:
              input.requestedDurationMinutes ?? current.durationMinutes,
