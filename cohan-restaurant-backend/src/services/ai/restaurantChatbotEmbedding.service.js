...
import {
  ...
  getLocalAiConfig,
  isLocalAiEnabled,
} from "./localAiProvider.service.js";
...
const normalizeEmbeddingProvider = () =>
  String(process.env.AI_EMBEDDING_PROVIDER || "")
    .trim()
    .toLowerCase();
const embeddingCacheTtlMs = () => {
  ...
  return Number.isFinite(parsed) && parsed > 0
    ? parsed
    : DEFAULT_EMBEDDING_CACHE_TTL_MS;
};
...
export async function createEmbedding(text) {
  ...
  try {
    ...
    const value = {
      ...
      model: result.model || config.embeddingModel,
      provider: "local",
    };
...
export function shouldRefreshEmbedding(item = {}) {
  ...
  if (normalizeEmbeddingProvider() !== "local" || !isLocalAiEnabled())
    return false;
...
export async function rebuildKnowledgeItemEmbedding(item) {
  ...
  if (!shouldRefreshEmbedding(item))
    return { updated: false, skipped: true, error: null, item };
  try {
    ...
    console.warn("[ai-chatbot] knowledge embedding refresh failed", {
      code: error?.code || "EMBEDDING_ERROR",
    });
    return {
      updated: false,
      skipped: false,
      error: error?.code || "embedding_error",
      item,
    };
  }
}

export async function rebuildRestaurantKnowledgeEmbeddings({
  restaurantId,
  ctx,
  ...
  const rows = await AiChatbotKnowledgeItem.find(query).sort({
    priority: -1,
    updatedAt: -1,
  });
  const result = {
...
export const rebuildRestaurantAiKnowledgeEmbeddings =
  rebuildRestaurantKnowledgeEmbeddings;

export const __testables = {
  embeddingTextForKnowledgeItem,
  normalizeText,
  hashText,
};
