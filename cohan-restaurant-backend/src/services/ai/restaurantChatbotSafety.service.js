...
const clampPriority = (v) =>
  Math.min(100, Math.max(0, Number.isFinite(Number(v)) ? Number(v) : 0));
...
const sanitizeRuleInput = (input = {}, { partial = false } = {}) => {
  ...
  if (!partial || input.ruleType != null) {
    ...
    if (!RULE_TYPES.has(ruleType))
      throw Object.assign(new Error("ruleType không hợp lệ"), {
        code: "BAD_USER_INPUT",
      });
  ...
  if (!partial || input.pattern != null) {
    ...
    if (!pattern)
      throw Object.assign(new Error("pattern là bắt buộc"), {
        code: "BAD_USER_INPUT",
      });
    patch.pattern = pattern;
  }
  if (!partial || input.responseMessage != null)
    patch.responseMessage = clean(input.responseMessage, 1000);
  if (!partial || input.enabled != null)
    patch.enabled = input.enabled == null ? true : Boolean(input.enabled);
  if (!partial || input.priority != null)
    patch.priority = clampPriority(input.priority);
  return patch;
};

export async function listRestaurantAiChatbotSafetyRules({
  restaurantId,
  filter = {},
  ctx,
}) {
...
export async function updateRestaurantAiChatbotSafetyRule({ input, ctx }) {
  ...
  Object.assign(found, sanitizeRuleInput(input, { partial: true }), {
    updatedBy: toObjectId(ctx?.user?.id || ctx?.user?._id),
  });
  await found.save();
...
export async function evaluateRestaurantAiChatbotSafety({
  restaurantId,
  message,
}) {
  ...
  const rules = await AiChatbotSafetyRule.find({
    restaurantId: rid,
    enabled: true,
  })
    .sort({ priority: -1, updatedAt: -1 })
  ...
  const disclaimers = matched
    .filter((r) => r.ruleType === "required_disclaimer")
    .map((r) => r.responseMessage || r.pattern)
    .filter(Boolean);
  const allowedScope = rules.filter((r) => r.ruleType === "allowed_scope");
  const inScope =
    !allowedScope.length ||
    allowedScope.some((r) => ruleMatches(r.pattern, text));
...
export async function bulkUpdateRestaurantAiChatbotSafetyRuleEnabled({
  ids = [],
  enabled,
  ctx,
}) {
...
export async function bulkDeleteRestaurantAiChatbotSafetyRules({
  ids = [],
  ctx,
}) {
  for (const id of ids) {
    if (mongoose.isValidObjectId(id))
      await deleteRestaurantAiChatbotSafetyRule({ id, ctx });
  }
  return true;
}
