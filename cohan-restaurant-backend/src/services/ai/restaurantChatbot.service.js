...
const publicRole = (user) => roleSlug(user) || user?.roleName || user?.userType || "customer";
...
const sanitizeFeatureMatches = (featureMatches = []) => (Array.isArray(featureMatches) ? featureMatches : [])
  .slice(0, 6)
  .map((item) => ({
  ...
  .filter((item) => item.key && item.label && item.path.startsWith("/"));
...
const buildContext = async ({ message, restaurantId, user, pageContext = {} }) => {
  ...
  const matchedFeatureMapEntries = sanitizeFeatureMatches(pageContext?.featureMatches || []);
  return {
    intent,
...
const fallbackActions = (context) => {
  ...
  for (const entry of context.matchedFeatureMapEntries || []) {
    ...
    actions.push({ type: entry.key === "cart" ? "openCart" : "link", label: entry.label, href: entry.path });
    if (actions.length >= 3) return actions;
  }
  if (context.intent === "cart") actions.push({ type: "openCart", label: "Mở giỏ hàng", href: "/cart" });
  if (context.intent === "checkout") actions.push({ type: "openCart", label: "Kiểm tra giỏ hàng", href: "/cart" });
