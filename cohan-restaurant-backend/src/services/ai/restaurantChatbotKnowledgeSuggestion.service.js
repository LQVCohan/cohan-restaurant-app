...
export async function approveRestaurantAiChatbotKnowledgeSuggestion({ id, input, ctx }) {
  ...
  if (found.status !== "pending") throw Object.assign(new Error("Suggestion chỉ có thể duyệt khi đang pending"), { code: "BAD_USER_INPUT" });
...
export async function dismissRestaurantAiChatbotKnowledgeSuggestion({ id, ctx }) {
  ...
  await ensurePermission(ctx, found.restaurantId, PERMISSIONS.RESTAURANT_WRITE);
  if (found.status !== "pending") throw Object.assign(new Error("Suggestion không thể dismiss ở trạng thái hiện tại"), { code: "BAD_USER_INPUT" });
