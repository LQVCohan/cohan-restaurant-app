...
const sanitizeCaseInput = (input, { partial = false } = {}) => {
  ...
  if (!partial && !question) {
    const e = new Error("BAD_USER_INPUT: question is required");
    e.code = "BAD_USER_INPUT";
    throw e;
  }
  const payload = {};
  if (!partial || input?.question !== undefined) payload.question = question;
  if (!partial || input?.expectedBehavior !== undefined)
    payload.expectedBehavior = asString(input?.expectedBehavior, 1000);
  if (!partial || input?.category !== undefined)
    payload.category = asString(input?.category, 80);
...
export const evaluateRestaurantAiChatbotPrompt = async ({ input, ctx }) => {
  ...
  if (!restaurantId || !message) {
    const e = new Error(
      "BAD_USER_INPUT: restaurantId and message are required",
    );
    e.code = "BAD_USER_INPUT";
    throw e;
  }
  ...
  const out = await handleRestaurantChatbotMessage({
    message,
    restaurantId,
  ...
  return {
    ...
    debug: input?.includeDebug
      ? {
          historyUsed: (input?.history || []).map((h) =>
            String(h?.content || ""),
          ),
          evaluationMode: true,
        }
...
export const listRestaurantAiChatbotEvaluationCases = async ({
  restaurantId,
  ctx,
}) => {
  await ensureReadEvaluationPermission(ctx, restaurantId);
  const rows = await AiChatbotEvaluationCase.find({
    restaurantId: asObjId(restaurantId),
...
export const createRestaurantAiChatbotEvaluationCase = async ({
  input,
  ctx,
}) => {
  ensureAuth(ctx);
  ...
  const created = await AiChatbotEvaluationCase.create({
    ...payload,
    restaurantId: asObjId(input.restaurantId),
    createdBy: uid,
    updatedBy: uid,
  });
  return toEvaluationCaseDto(created);
};
export const updateRestaurantAiChatbotEvaluationCase = async ({
  input,
  ctx,
}) => {
  ensureAuth(ctx);
...
export const runRestaurantAiChatbotEvaluationSet = async ({ input, ctx }) => {
  ...
  if (Array.isArray(input?.caseIds) && input.caseIds.length)
    filter._id = {
      $in: input.caseIds
        .filter((x) => mongoose.isValidObjectId(x))
        .map((x) => new mongoose.Types.ObjectId(x)),
    };
  const cases = await AiChatbotEvaluationCase.find(filter)
    .sort({ updatedAt: -1 })
    .lean();
  const results = [];
  for (const c of cases) {
    ...
    results.push({
      ...
      debug: input?.includeDebug ? { evaluationMode: true } : null,
    });
  }
  return results;
};
