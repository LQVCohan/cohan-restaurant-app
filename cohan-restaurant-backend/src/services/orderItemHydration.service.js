...
async function assertMenuItemsAvailable({ restaurantId, items, session }) {
  ...
  const ids = [...new Set((items || []).map(resolveMenuItemId).filter(Boolean).map(String))]
    .map(toObjectId)
    .filter(Boolean);

  if (!ids.length || ids.length !== new Set((items || []).map(resolveMenuItemId).filter(Boolean).map(String)).size) {
