...
const SAFE_FIELDS = Object.freeze(["_id", "id", "name", "slug", "code", "permissions", "parentRole", "department", "isSystem", "isActive", "action", "resource", "group", "description"]);
const SENSITIVE_KEYS = new Set(["password", "passwordHash", "token", "accessToken", "refreshToken", "secret", "authorization"]);
...
export function sanitizeAuditPayload(payload) {
  ...
  for (const field of fields) {
    if (source[field] === undefined || SENSITIVE_KEYS.has(field)) continue;
    output[field === "_id" ? "id" : field] = simplifyValue(source[field]);
  }
  return output;
}
...
function getUserAgent(ctx) {
  return ctx?.userAgent || ctx?.request?.headers?.["user-agent"] || ctx?.req?.headers?.["user-agent"] || undefined;
}

export async function logRbacAudit({ ctx = {}, action, targetType, targetId, targetName, restaurantId, before, after, metadata }) {
  try {
    ...
    return await AuditLog.create({
      ...
      byUserId: actorId,
      diff: { before: sanitizedBefore ?? null, after: sanitizedAfter ?? null },
    });
  } catch (error) {
