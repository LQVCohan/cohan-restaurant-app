...
import {
  ...
  cleanupExpiredTableViewLocks,
} from "../services/reservationAutoCancel.service.js";
...
import {
  applyPaymentProviderCallback,
  createReservationPayment,
  getPaymentSessionById,
  listReservationPayments,
  reconcileBankTransferWebhook,
} from "../services/payment/paymentSession.service.js";
import {
  isVnpaySuccessful,
  verifyMomoCallback,
  verifyVnpayCallback,
...
import {
  validateGuestConversationOwnership,
  isValidConversationId,
  getAiConversationGuestRoomName,
} from "../services/ai/restaurantChatbotRealtime.service.js";
import {
  AI_CHATBOT_RATE_LIMIT_POLICIES,
  consumeAiChatbotRateLimit,
} from "../services/ai/restaurantChatbotRateLimit.service.js";
...
export async function authorizeChatThreadJoin({
  socketUser,
  threadId,
  findThreadById,
  requireRestaurantPermissionFn,
  permissionCode,
}) {
  ...
  try {
    await requireRestaurantPermissionFn(
      { user: socketUser },
      thread.restaurantId,
      permissionCode,
    );
...
export function buildPaymentReturnPage({
  provider,
  verified,
  successful,
  paymentFound,
  ...
  const providerLabel =
    String(provider || "").toLowerCase() === "momo" ? "MoMo" : "VNPAY";
  let title = "Đã ghi nhận kết quả thanh toán";
  let message =
    "Vui lòng quay lại cửa sổ COHAN. Hệ thống sẽ tự động cập nhật khi cổng thanh toán gửi xác nhận.";

  if (!verified) {
    title = "Không thể xác thực kết quả thanh toán";
    message =
      "Dữ liệu trả về không hợp lệ. Vui lòng quay lại COHAN và kiểm tra trạng thái giao dịch trước khi thử lại.";
  } else if (!paymentFound) {
    title = "Không tìm thấy phiên thanh toán";
    message =
      "COHAN chưa xác định được giao dịch tương ứng. Vui lòng quay lại ứng dụng để kiểm tra.";
  } else if (!successful) {
    title = "Giao dịch chưa hoàn tất";
    message =
      "Cổng thanh toán chưa xác nhận giao dịch thành công. Vui lòng quay lại COHAN để thử lại hoặc chọn phương thức khác.";
  }
...
export function getVnpayIpnRejection(payment, payload = {}) {
  if (!payment || String(payment.callbackStatus || "").toLowerCase() !== "rejected") {
    return null;
  }

  const providerAmount = Math.round(Number(payload.vnp_Amount || 0) / 100);
  const expectedAmount = Math.round(Number(payment.amount || 0));
  if (providerAmount > 0 && providerAmount !== expectedAmount) {
...
export function shouldAllowAuthCookieRequestOrigin({
  origin,
  allowedOrigins,
  nodeEnv,
  allowNoOriginValue,
  ...
  const allowNoOrigin =
    String(allowNoOriginValue || "").toLowerCase() === "true";
...
export async function createServer() {
  ...
  const allowUnsafeInlineStyle =
    String(process.env.CSP_ALLOW_UNSAFE_INLINE_STYLE || "false").toLowerCase() ===
    "true";
  await app.register(helmet, {
    contentSecurityPolicy: buildContentSecurityPolicyDirectives({
      inProduction,
      allowedOrigins,
      s3PublicBase,
      allowUnsafeInlineStyle,
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!currentToken) {
        ...
        return reply
          .code(401)
          .send({ ok: false, message: "Authentication failed" });
      }
  ...
  app.post(
    "/api/payments/reservations/:reservationId/create",
    async (req, reply) => {
      try {
  ...
  app.get("/api/payments/:paymentId/status", async (req, reply) => {
    try {
      ...
      const isOwner =
        authUser?.id && String(authUser.id) === String(payment.userId);
      if (!isOwner) {
  ...
  app.get(
    ...
    async (req, reply) => {
      try {
        ...
        const list = await listReservationPayments(
        ...
        return reply.code(400).send({
          ok: false,
          message: err?.message || "List payments failed",
        });
      }
    },
  );
  ...
  app.post(
    "/api/payments/webhooks/bank-transfer/:provider",
    async (req, reply) => {
      try {
        const inProduction = process.env.NODE_ENV === "production";
        const hmacSecret = String(
          process.env.BANK_TRANSFER_WEBHOOK_HMAC_SECRET || "",
        );
        const expectedStaticSecret = String(
          process.env.BANK_TRANSFER_WEBHOOK_SECRET || "",
        );
        const toleranceSeconds = Math.max(
          1,
          Number(
            process.env.BANK_TRANSFER_WEBHOOK_TIMESTAMP_TOLERANCE_SECONDS ||
              300,
          ),
        );
        const timestampHeader = String(
          req.headers["x-bank-webhook-timestamp"] || "",
        ).trim();
        const signatureHeader = String(
          req.headers["x-bank-webhook-signature"] || "",
        )
          .trim()
          .toLowerCase();
        const staticSecretHeader = String(
        ...
        if (hmacSecret) {
          ...
          if (
            ...
            return reply
              .code(401)
              .send({ ok: false, message: "Unauthorized webhook" });
          }
          const skew = Math.abs(
            Math.floor(Date.now() / 1000) - Math.floor(timestampSeconds),
          );
        ...
        if (
          !authorized ||
          (inProduction && !hmacSecret && !expectedStaticSecret)
        ) {
          req.log.warn(
            { provider: req.params?.provider },
            "bank transfer webhook rejected",
          );
          return reply
            .code(401)
            .send({ ok: false, message: "Unauthorized webhook" });
        }
        const result = await reconcileBankTransferWebhook({
          provider: req.params?.provider || "bank_transfer",
          payload: req.body || {},
        });
  ...
  app.get("/api/payments/webhooks/vnpay", async (req, reply) => {
    try {
      ...
      const rejection = getVnpayIpnRejection(payment, req.query || {});
      if (rejection) return reply.code(200).send(rejection);
      ...
      return reply
        .code(200)
        .send({ RspCode: "00", Message: "Confirm Success" });
    } catch (err) {
      ...
      if (message.includes("amount")) {
        return reply
          .code(200)
          .send({ RspCode: "04", Message: "Invalid Amount" });
      }
      return reply
        .code(200)
        .send({ RspCode: "99", Message: "Unknown error" });
    }
  });
  ...
  app.get("/api/payments/return/:provider", async (req, reply) => {
    ...
    const reference =
      provider === "momo" ? payload.orderId : payload.vnp_TxnRef;
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (invalidCoordinate) {
        return reply
          .code(400)
          .send({ ok: false, message: "lat,lng không hợp lệ" });
      }
      ...
      try {
        ...
        const streetName =
          ...
          addr.neighbourhood ||
          "";
        const street = [addr.house_number, streetName]
          .filter(Boolean)
          .join(" ")
          .trim();
  ...
  app.post(
    "/api/ai/table/merge-suggestion",
    {
      config: {
        rateLimit: {
          max: Number(process.env.RL_AI_TABLE_MAX || 30),
          timeWindow: process.env.RL_AI_TABLE_WINDOW || "1 minute",
        },
      },
    },
    async (req, reply) => {
      const payload = await aiRouteGuard(
        req,
        reply,
  ...
  app.post(
    "/api/ai/table/turnover-prediction",
    {
      config: {
        rateLimit: {
          max: Number(process.env.RL_AI_TABLE_MAX || 30),
  ...
  app.post("/api/ai/floor/generate-layout", async (req, reply) => {
    ...
    if (!restaurantId) {
      return reply
        .code(400)
        .send({ ok: false, message: "restaurantId is required" });
    }
    const authUser = await resolveAuthenticatedUserFromRequest(req);
    ...
    try {
      ...
      req.log.warn(
        { err, restaurantId, userId: String(userId) },
        "Forbidden floor layout generation",
      );
      return reply.code(403).send({ ok: false, message: "Forbidden" });
    }
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("joinUserChannel", (userId, ack) => {
      const isAdmin = ["admin"].includes(
        String(socket.user?.roleName || "").toLowerCase(),
      );
      if (
        !socket.user?.id ||
        (!isAdmin && String(socket.user.id) !== String(userId))
      ) {
        if (typeof ack === "function") {
          ack({ ok: false, code: "FORBIDDEN" });
        }
    ...
    socket.on("joinAiChatbotConversation", async (payload = {}, ack) => {
      try {
        ...
        if (!rateResult.allowed) {
          if (typeof ack === "function") {
            ack({ ok: false, code: rateResult.code });
          }
          return;
        }
        ...
        if (typeof ack === "function") ack({ ok: true });
        socket.emit("joinedAiChatbotConversation", {
          ok: true,
    ...
    socket.on("leaveMenuItemView", ({ restaurantId, menuItemId }) => {
      ...
      const cur = Math.max(
        0,
        Number(app.menuPresenceStore.get(key) || 0) - 1,
      );
  ...
  if (process.env.ENABLE_ATTENDANCE_EXCEPTION_JOB === "true") {
    cron.schedule("*/30 * * * *", async () => {
      try {
        const result =
          await runAttendanceExceptionDetectionForAllRestaurants({
            triggeredBy: "system",
          });
...
const aiRouteGuard = async (req, reply, permissionCode) => {
  ...
  if (!restaurantId || !mongoose.isValidObjectId(String(restaurantId))) {
    reply.code(400).send({
      ok: false,
      message: "restaurantId is required and must be valid",
    });
    return null;
  }
  const authUser = await resolveAuthenticatedUserFromRequest(req);
