...
const PROFILES = [
  ...
  [
    ...
    "2025-07-14",
    9000000,
    "105 Đường Nguyễn Thị Minh Khai",
    "Phường Bến Thành",
    "Quận 1",
  ],
  [
...
async function assertNoConflicts() {
  ...
  const phoneByEmail = new Map(
    PROFILES.map((profile) => [profile.email, profile.phone]),
  );
  const codeByEmail = new Map(
    PROFILES.map((profile) => [profile.email, profile.employeeCode]),
  );
  ...
  const [phoneOwners, codeOwners] = await Promise.all([
    User.find({ phone: { $in: PROFILES.map((profile) => profile.phone) } })
      ...
      .lean(),
    Staff.find({
      employeeCode: { $in: PROFILES.map((profile) => profile.employeeCode) },
    })
  ...
  const phoneConflict = phoneOwners.find(
    (owner) =>
      !emails.includes(owner.email) ||
      phoneByEmail.get(owner.email) !== owner.phone,
  );
  if (phoneConflict) {
    throw new Error(
      `DEMO_PROFILE_PHONE_CONFLICT: ${phoneConflict.phone} (${phoneConflict.email || "unknown"})`,
    );
  }
  ...
  const codeConflict = codeOwners.find(
    (owner) =>
      !emails.includes(owner.email) ||
      codeByEmail.get(owner.email) !== owner.employeeCode,
  );
  if (codeConflict) {
    throw new Error(
      `DEMO_PROFILE_EMPLOYEE_CODE_CONFLICT: ${codeConflict.employeeCode} (${codeConflict.email || "unknown"})`,
    );
  }
}
...
async function main() {
  ...
  const missing = PROFILES.map((profile) => profile.email).filter(
  ...
  for (const profile of PROFILES) {
    ...
    await Staff.updateOne(
      {
        _id: staffByEmail.get(profile.email)._id,
        restaurantForStaff: restaurantId,
      },
      {
        $set: {
