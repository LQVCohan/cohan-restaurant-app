...
async function run() {
  ...
  console.log(`Verifying attendance as-of=${DEMO_AS_OF_DATE}, cutoff=${attendanceCutoffDate || "none"}`);
  ...
  const [shifts, timesheets, corrections, publications, snapshots, overtimeRequests] = await Promise.all([
    Shift.find({
      restaurantId,
  ...
  const regularTimesheets = timesheets.filter((item) => !item.isOffSchedule && item.shiftId);
  const offScheduleTimesheets = timesheets.filter((item) => item.isOffSchedule);
  const overtimeTimesheets = regularTimesheets.filter((item) => Number(item.overtimeMinutes || 0) > 0);
  const expectedRegularTimesheets = attendanceDates.length * STAFF_LEVELS.length;

  check(shifts.length === allDates.length * STAFF_LEVELS.length, "created 98 shifts for fourteen days and seven employees");
  check(regularTimesheets.length === expectedRegularTimesheets, `created ${expectedRegularTimesheets} regular timesheets only for completed dates`);
  check(offScheduleTimesheets.length === 3, "created three off-schedule attendance review examples");
  check(timesheets.length === expectedRegularTimesheets + 3, "total attendance count contains no future placeholders");
  check(corrections.length === 16, "created fourteen countable corrections plus two cancelled examples");
  ...
  check([...dates.values()].every((count) => count === 7), "every roster date contains seven employee shifts");
  ...
  check(regularDates.size === attendanceDates.length, "attendance covers only completed calendar dates");
  check([...regularDates.keys()].every((date) => date < DEMO_AS_OF_DATE), "no regular attendance exists today or in the future");
  check([...regularDates.values()].every((count) => count === 7), "every completed date contains seven regular attendance rows");
  check(regularEmployeeDates.size === expectedRegularTimesheets, "there are no duplicate regular employee/date attendance rows");
  ...
  for (const [email] of STAFF_LEVELS) {
    ...
    check(
      Number(regularByEmployee.get(String(employee?._id)) || 0) === attendanceDates.length,
      `${email} has attendance for every completed date`,
    );
  }

  const correctionStatuses = new Set(corrections.map((item) => item.status));
  check(correctionStatuses.has("pending"), "attendance corrections include pending review");
  check(correctionStatuses.has("applied"), "attendance corrections include applied approvals");
  check(correctionStatuses.has("rejected"), "attendance corrections include rejected reviews");
  check(correctionStatuses.has("cancelled"), "attendance corrections include employee-cancelled requests");
  check(corrections.filter((item) => item.status !== "cancelled").length === 14, "fourteen corrections remain performance-countable");
  check(corrections.every((item) => hcmDate(item.workDate) < DEMO_AS_OF_DATE), "all correction requests are for completed dates");
  check(
    corrections.every((item) => [item.requestedAt, item.reviewedAt, item.appliedAt].every(isBeforeAsOf)),
  ...
  check(offScheduleStatuses.has("pending"), "off-schedule attendance includes pending review");
  check(offScheduleStatuses.has("approved"), "off-schedule attendance includes approved review");
  check(offScheduleStatuses.has("rejected"), "off-schedule attendance includes rejected review");
  check(offScheduleTimesheets.every((item) => !item.shiftId), "off-schedule attendance is not bound to roster shifts");
  check(
    offScheduleTimesheets.every((item) => [item.actualCheckInAt, item.actualCheckOutAt, item.offScheduleReviewedAt].every(isBeforeAsOf)),
  ...
  check(overtimeTimesheets.length === 5, "created five overtime-bearing timesheets");
  check(overtimeApprovalStatuses.has("pending"), "timesheet overtime includes pending review");
  check(overtimeApprovalStatuses.has("approved"), "timesheet overtime includes approved review");
  check(overtimeApprovalStatuses.has("rejected"), "timesheet overtime includes rejected review");
  check(
    overtimeTimesheets.every((item) => item.actualCheckOutAt > item.plannedEndTime),
  ...
  check(overtimeRequests.length === 5, "created five employee overtime requests");
  check(overtimeRequests.every((item) => hcmDate(item.workDate) < DEMO_AS_OF_DATE), "all overtime requests are for completed dates");
  check(
    overtimeRequests.every((item) =>
      [item.requestedAt, item.employeeConfirmedAt, item.approvedAt, item.rejectedAt, item.completedAt].every(isBeforeAsOf),
    ),
  ...
  for (const [email, expectedLevel] of STAFF_LEVELS) {
    ...
    check(snapshot?.factors?.attendanceDataAsOf === DEMO_AS_OF_DATE, `${email} snapshot records attendance as-of date`);
    check(snapshot?.factors?.attendanceDataCutoff === attendanceCutoffDate, `${email} snapshot records attendance cutoff`);
  }
