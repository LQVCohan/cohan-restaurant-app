...
async function run() {
  ...
  check(roster.shifts.length > 0, "found the existing previous/current week roster");
  check(missingStaffIds.length === 0, "every roster employee resolves to an existing staff account");
  ...
  for (const shift of roster.shifts) {
    ...
    if (
      ...
      durationMinutes(shift.startTime, shift.endTime) === plan.shiftHours * 60 &&
  ...
  check(eligibleShiftCount > 0, "found full-time or part-time shifts to validate");
  check(
    validShiftCount === eligibleShiftCount,
    "all full-time and part-time shifts use 8-hour/4-hour windows inside 07:00-23:00",
  );

  const sameEmployeeOverlap = [...shiftsByEmployeeDate.values()].some((rows) => {
    const sorted = [...rows].sort(
      (left, right) => new Date(left.startTime) - new Date(right.startTime),
    );
    return sorted.some(
      (row, index) =>
        index > 0 && new Date(row.startTime) < new Date(sorted[index - 1].endTime),
    );
  });
  check(!sameEmployeeOverlap, "no employee receives overlapping shifts on the same date");
  ...
  check(mixedDates.length > 0, "at least one roster date contains both employment types");
  check(
