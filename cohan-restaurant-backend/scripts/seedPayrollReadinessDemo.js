...
const at = (base, dayOffset, hour = 0, minute = 0) =>
  new Date(Date.UTC(base.getUTCFullYear(), base.getUTCMonth(), base.getUTCDate() + dayOffset, hour, minute, 0, 0));
...
async function upsertStaff({ email, fullName, roleId, restaurantId, code, department, positionTitle, baseSalary }) {
  const passwordHash = await bcrypt.hash(DEMO_PASSWORD, 10);
...
async function resolveDemoRestaurant(managerId = null) {
  const restaurant = await Restaurant.findOneAndUpdate(
    { name: "Cohan Payroll Readiness Demo", description: { $regex: DEMO_TAG } },
    {
      $set: {
...
async function resetScenario(restaurantId) {
  if (!RESET) return;
  await Promise.all([
    Shift.deleteMany({ restaurantId, notes: { $regex: DEMO_TAG } }),
    Timesheet.deleteMany({ restaurantId, note: { $regex: DEMO_TAG } }),
    AttendanceCorrectionRequest.deleteMany({ restaurantId, note: { $regex: DEMO_TAG } }),
    OvertimeRequest.deleteMany({ restaurantId, note: { $regex: DEMO_TAG } }),
    SchedulePublication.deleteMany({ restaurantId, notes: { $regex: DEMO_TAG } }),
    ShiftAcknowledgement.deleteMany({ note: { $regex: DEMO_TAG } }),
    ScheduleAcknowledgement.deleteMany({ restaurantId, note: { $regex: DEMO_TAG } }),
    PayrollItem.deleteMany({ restaurantId, employeeName: { $regex: "Demo Payroll" } }),
    PayrollPeriod.deleteMany({ restaurantId, name: { $regex: "Payroll Readiness Demo" } }),
  ]);
}

async function upsertPayrollSettings({ restaurantId, currentPayrollPeriodId, actorId }) {
...
async function upsertPublication({ restaurantId, managerId, startDate, status = "published" }) {
  return SchedulePublication.findOneAndUpdate(
    { restaurantId, periodStart: startDate, periodEnd: endOfWeek(startDate) },
    {
      $set: {
        ...
        notes: `${DEMO_TAG} ${status} schedule publication`,
      },
    },
    { upsert: true, new: true },
  );
...
async function upsertShift({ restaurantId, employeeId, start, end, shiftType = "morning" }) {
...
async function upsertScheduleAck({ restaurantId, publicationId, employeeId, status = "acknowledged" }) {
  return ScheduleAcknowledgement.findOneAndUpdate(
    { restaurantId, schedulePublicationId: publicationId, employeeId },
    {
      $set: {
        ...
        note: `${DEMO_TAG} schedule acknowledgement`,
      },
    },
    { upsert: true, new: true },
  );
...
async function upsertTimesheet({ restaurantId, employeeId, shiftId = null, workDate, plannedStart, plannedEnd, checkIn, checkOut, status = "completed", isOffSchedule = false, approved = true, extra = {} }) {
  return Timesheet.findOneAndUpdate(
    { restaurantId, employeeId, workDate, ...(shiftId ? { shiftId } : { isOffSchedule: true }) },
    {
      $set: {
        restaurantId,
        employeeId,
        ...
        status,
        source: "quick",
        workedMinutes: checkIn && checkOut ? Math.max(Math.round((checkOut - checkIn) / 60000), 0) : 0,
        hours: checkIn && checkOut ? Number((((checkOut - checkIn) / 60000) / 60).toFixed(2)) : 0,
        approved,
        isOffSchedule,
...
async function seedReadyScenario({ restaurant, manager, staff }) {
  ...
  for (const [index, employee] of staff.entries()) {
    ...
    await upsertScheduleAck({ restaurantId: restaurant._id, publicationId: publication._id, employeeId: employee._id });
    ...
    await upsertPayrollItem({ periodId: period._id, restaurantId: restaurant._id, staff: employee });
  }
...
async function seedBlockedScenario({ restaurant, staff }) {
  ...
  await AttendanceCorrectionRequest.findOneAndUpdate(
    { restaurantId: restaurant._id, employeeId: cleanStaff._id, workDate: at(BLOCKED_START, 2, 0, 0), status: "pending" },
    {
      $setOnInsert: {
        restaurantId: restaurant._id,
        employeeId: cleanStaff._id,
        ...
        note: `${DEMO_TAG} pending correction`,
      },
    },
    { upsert: true, new: true },
  );
  ...
  await OvertimeRequest.findOneAndUpdate(
    { restaurantId: restaurant._id, employeeId: issueStaff._id, workDate: at(BLOCKED_START, 3, 0, 0), status: "approved" },
    {
      $setOnInsert: {
        restaurantId: restaurant._id,
        employeeId: issueStaff._id,
        ...
        status: "approved",
        note: `${DEMO_TAG} approved but not completed overtime`,
      },
    },
    { upsert: true, new: true },
  );
...
async function main() {
  ...
  const manager = await upsertUser({
    ...
    extra: { restaurantForStaff: restaurant._id, refRestaurants: [restaurant._id] },
  });
  ...
  const readyPeriod = await seedReadyScenario({ restaurant, manager, staff: [cleanStaff, issueStaff] });
  const blockedPeriod = await seedBlockedScenario({ restaurant, staff: [cleanStaff, issueStaff] });
  await upsertPayrollSettings({ restaurantId: restaurant._id, currentPayrollPeriodId: blockedPeriod._id, actorId: accountant._id });

  console.log("Seeded payroll readiness demo data successfully.");
  console.log("Restaurant:", String(restaurant._id), restaurant.name);
  console.log("Ready period:", String(readyPeriod._id), readyPeriod.name, readyPeriod.startDate.toISOString(), readyPeriod.endDate.toISOString());
  console.log("Blocked period:", String(blockedPeriod._id), blockedPeriod.name, blockedPeriod.startDate.toISOString(), blockedPeriod.endDate.toISOString());
