...
describe("kitchenOrderWorkItem service", () => {
  ...
  it("preparing -> ready computes actualPrepMinutes with target and on_time", async () => {
    ...
    expect(modelMocks.KitchenOrderWorkItem.findOneAndUpdate).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        $set: expect.objectContaining({ actualPrepMinutes: 15, targetPrepMinutes: 20, timeLevel: "on_time" }),
      }),
      expect.anything(),
    );
  });
  ...
  it("ready computes very_late when prep duration exceeds late grace", async () => {
    ...
    expect(modelMocks.KitchenOrderWorkItem.findOneAndUpdate).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        $set: expect.objectContaining({ actualPrepMinutes: 40, targetPrepMinutes: 20, timeLevel: "very_late" }),
      }),
      expect.anything(),
    );
  });
  ...
  it("ready keeps existing targetPrepMinutes and does not overwrite", async () => {
    ...
    expect(modelMocks.KitchenOrderWorkItem.findOneAndUpdate).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        $set: expect.objectContaining({ actualPrepMinutes: 14, targetPrepMinutes: 12, timeLevel: "late" }),
      }),
      expect.anything(),
    );
  });
