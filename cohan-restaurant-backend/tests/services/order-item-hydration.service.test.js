...
describe("hydrateCheckoutOrderItems", () => {
  ...
  it("snapshots prepStation from the persisted menu item", async () => {
    seedBase({ prepStation: "bar" });
    ...
    expect(item.prepStation).toBe("bar");
  });

  it("rejects checkout when the persisted menu item has no station", async () => {
    seedBase({ prepStation: null });
