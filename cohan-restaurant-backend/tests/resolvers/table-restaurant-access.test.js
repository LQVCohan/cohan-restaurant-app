...
describe("table restaurant access guards", () => {
  ...
  it("tables allowed calls guard and find", async () => { const q=(await import("../../graphql/resolvers/table/query.js")).default; await q.tables(null,{restaurantId:"valid-r1"},{}); expect(guardMocks.requireRestaurantAccess).toHaveBeenCalledWith({},"valid-r1"); expect(tableMocks.find).toHaveBeenCalled(); });
  it("createTable allowed guard before create", async () => { const m=(await import("../../graphql/resolvers/table/mutation.js")).default; await m.createTable(null,{input:{restaurantId:"valid-r1",floorId:"valid-f1",code:"A"}},{}); expect(guardMocks.requireRestaurantAccess.mock.invocationCallOrder[0]).toBeLessThan(tableMocks.create.mock.invocationCallOrder[0]); });
  it("setTableStatus allowed updates", async () => { const m=(await import("../../graphql/resolvers/table/mutation.js")).default; await m.setTableStatus(null,{input:{id:"valid-t1",status:"OPEN"}},{}); expect(tableMocks.findByIdAndUpdate).toHaveBeenCalled(); });

  it("updateTable rejects cross-restaurant floor", async () => { const m=(await import("../../graphql/resolvers/table/mutation.js")).default; floorMocks.findById.mockReturnValueOnce(selectLeanWrap({ restaurantId:"valid-r2", level:2 })); await expect(m.updateTable(null,{input:{id:"valid-t1",floorId:"valid-floor-2"}},{})).rejects.toThrow("Floor does not belong to this restaurant"); expect(tableMocks.findByIdAndUpdate).not.toHaveBeenCalled(); });
  it("moveTable rejects cross-restaurant floor", async () => { const m=(await import("../../graphql/resolvers/table/mutation.js")).default; floorMocks.findById.mockReturnValueOnce(selectLeanWrap({ restaurantId:"valid-r2", level:2 })); await expect(m.moveTable(null,{input:{id:"valid-t1",floorId:"valid-floor-2"}},{})).rejects.toThrow("Floor does not belong to this restaurant"); expect(tableMocks.findByIdAndUpdate).not.toHaveBeenCalled(); });
});
