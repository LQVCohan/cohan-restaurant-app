...
vi.mock(
  "../../src/services/auth/restaurantScope.service.js",
...
vi.mock("../../src/services/scheduling/schedulingPolicy.service.js", () => ({
  getSchedulingPolicy: vi.fn().mockResolvedValue({
    ...
    employmentTypePolicy: {
      ...
      seasonal: { requireAvailability: true, minWeeklyHours: 0 },
      probation: { requireAvailability: false, minWeeklyHours: 0 },
      contract: { requireAvailability: false, minWeeklyHours: 0 },
    },
...
describe("availability resolver", () => {
  beforeEach(() => {
    ...
    modelMocks.Staff.findById.mockReturnValue(staffQuery(defaultStaff));
  });
  ...
  it("creates availability window successfully", async () => {
    ...
    modelMocks.AvailabilityRegistrationWindow.create.mockResolvedValue({
      _id: "w1",
      status: "draft",
    });
    const res = await mutation.createAvailabilityWindow(
      ...
      { user: { id: "u1", roles: ["manager"], restaurantId: "r1" } },
    );
  ...
  it("defaults and overrides lateChangeRequiresApproval on createAvailabilityWindow", async () => {
    ...
    await mutation.createAvailabilityWindow(
      null,
      {
        input: {
          restaurantId: "r1",
    ...
    expect(
      modelMocks.AvailabilityRegistrationWindow.create.mock.calls[0][0]
    ...
    await mutation.createAvailabilityWindow(
      null,
      {
        input: {
          ...
          periodStart: new Date(),
          periodEnd: new Date(),
          lateChangeRequiresApproval: false,
        },
      },
      { user: { id: "u1", roles: ["manager"], restaurantId: "r1" } },
    );
    expect(
      modelMocks.AvailabilityRegistrationWindow.create.mock.calls[1][0]
        .lateChangeRequiresApproval,
    ).toBe(false);
  });

  it("handles duplicate window by returning existing", async () => {
    const { createOrGetAvailabilityRegistrationWindow } = await import(
      "../../src/services/availability/availabilityRegistrationWindow.service.js"
    );
    modelMocks.AvailabilityRegistrationWindow.create.mockRejectedValue({
      code: 11000,
    });
    modelMocks.AvailabilityRegistrationWindow.findOne.mockResolvedValue({
      _id: "existing",
    });
    const res = await createOrGetAvailabilityRegistrationWindow(
      {
        restaurantId: "r1",
  ...
  it("stores part-time weekly availability slots", async () => {
    ...
    modelMocks.StaffAvailabilitySubmission.findOneAndUpdate.mockResolvedValue({
  ...
  it("stores full-time unavailable exception", async () => {
    ...
    modelMocks.Staff.findById.mockReturnValueOnce(
      staffQuery({
        _id: "e1",
        restaurantForStaff: "r1",
    ...
    expect(
      modelMocks.StaffAvailabilitySubmission.findOneAndUpdate.mock.calls[0][1]
        .$set.employmentType,
    ).toBe("full_time");
  });
  ...
  it("blocks direct submit after close when late change is disabled", async () => {
    ...
    ).rejects.toThrow("AVAILABILITY_WINDOW_CLOSED");
  });
  ...
  it("sets previousStatusBeforeLateChange from existing locked submission", async () => {
    ...
    expect(
      modelMocks.StaffAvailabilitySubmission.findOneAndUpdate.mock.calls[0][1]
        .$set.previousStatusBeforeLateChange,
    ).toBe("locked");
  });
  ...
  it("rejecting late change restores previous official status and keeps official slots", async () => {
    ...
    modelMocks.StaffAvailabilitySubmission.findById.mockResolvedValue({
      ...
      slots: [
        { date: new Date(), shiftType: "morning", status: "available" },
      ],
      pendingSlots: [
        { date: new Date(), shiftType: "evening", status: "available" },
      ],
    });
    modelMocks.StaffAvailabilitySubmission.findByIdAndUpdate.mockResolvedValue({
      _id: "s1",
      status: "locked",
    });
    const res = await mutation.reviewStaffAvailabilitySubmission(
      null,
      { input: { id: "s1", status: "rejected", reviewNote: "x" } },
      { user: { id: "m1", roles: ["manager"], restaurantId: "r1" } },
    );
    expect(res.status).toBe("locked");
    expect(
      modelMocks.StaffAvailabilitySubmission.findByIdAndUpdate,
  ...
  it("allows manager in scope to open close and cancel availability window", async () => {
    ...
    const openRes = await mutation.openAvailabilityWindow(
      null,
      { id: "w1" },
      ctx,
    );
    const closeRes = await mutation.closeAvailabilityWindow(
      null,
      { id: "w1" },
      ctx,
    );
    const cancelRes = await mutation.cancelAvailabilityWindow(
      null,
      { id: "w1", reason: "ops" },
      ctx,
    );
    ...
    expect(
      modelMocks.AvailabilityRegistrationWindow.findById,
    ).toHaveBeenCalledTimes(3);
    expect(
      modelMocks.StaffAvailabilitySubmission.updateMany,
  ...
  it("blocks manager outside restaurant scope for open close and cancel", async () => {
    ...
    await expect(
      mutation.openAvailabilityWindow(null, { id: "w1" }, ctx),
    ).rejects.toThrow("FORBIDDEN_SCOPE");
    await expect(
      mutation.closeAvailabilityWindow(null, { id: "w1" }, ctx),
    ).rejects.toThrow("FORBIDDEN_SCOPE");
    await expect(
  ...
  it("returns AVAILABILITY_WINDOW_NOT_FOUND for open close and cancel", async () => {
    ...
    await expect(
      mutation.openAvailabilityWindow(null, { id: "missing" }, ctx),
    ).rejects.toThrow("AVAILABILITY_WINDOW_NOT_FOUND");
    await expect(
      mutation.closeAvailabilityWindow(null, { id: "missing" }, ctx),
    ).rejects.toThrow("AVAILABILITY_WINDOW_NOT_FOUND");
    await expect(
      mutation.cancelAvailabilityWindow(
        null,
  ...
  it("closeAvailabilityWindow sets closedAt and locks submitted/approved submissions", async () => {
    ...
    modelMocks.AvailabilityRegistrationWindow.findByIdAndUpdate.mockResolvedValue({
      _id: "w1",
      status: "closed",
      closeAt: deadline,
      closedBy: "m1",
    ...
    const res = await mutation.closeAvailabilityWindow(
      null,
      { id: "w1" },
      ctx,
    );
  ...
  it("allows staff to view their own submission", async () => {
    ...
    modelMocks.StaffAvailabilitySubmission.findOne.mockResolvedValue({
      _id: "s1",
      employeeId: "e1",
    });

    const res = await query.staffAvailabilitySubmission(
      null,
    ...
    expect(modelMocks.StaffAvailabilitySubmission.findOne).toHaveBeenCalledWith({
      availabilityWindowId: "w1",
      employeeId: "e1",
    });
  });
  ...
  it("allows HR but blocks accountant from submission list", async () => {
    ...
    modelMocks.StaffAvailabilitySubmission.find.mockResolvedValue([
      { _id: "s1" },
    ]);
    await expect(
      query.staffAvailabilitySubmissions(
  ...
  it("writes pending slots for closed late-change submit without overwriting official slots", async () => {
    ...
    const update =
      modelMocks.StaffAvailabilitySubmission.findOneAndUpdate.mock.calls.at(-1)[1];
  ...
  it("approves late change by copying pendingSlots into slots and clearing pending fields", async () => {
    ...
    const set =
      modelMocks.StaffAvailabilitySubmission.findByIdAndUpdate.mock.calls[0][1]
        .$set;
