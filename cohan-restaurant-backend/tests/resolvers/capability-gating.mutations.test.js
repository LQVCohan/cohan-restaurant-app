...
const guards = vi.hoisted(() => ({
  ...
  assertRestaurantCanReserve: vi.fn(),
}));

vi.mock("../../models/index.js", () => model);
vi.mock("mongoose", async (importOriginal) => {
  const actual = await importOriginal();
  const ActualObjectId = actual.default.Types.ObjectId;
  class ObjectId extends ActualObjectId {
    constructor(value) {
      super(
        actual.default.isValidObjectId(value)
          ? value
    ...
    toString() {
      return this.mockValue == null
        ? super.toString()
        : String(this.mockValue);
    }
  }
...
const sessionQuery = (value) => ({
  ...
  sort: vi.fn(() => ({
    lean: vi.fn().mockResolvedValue(value),
    session: vi.fn(() => ({ lean: vi.fn().mockResolvedValue(value) })),
  })),
});
...
beforeEach(() => {
  ...
  model.Menu.findOne.mockReturnValue({
    lean: vi.fn().mockResolvedValue({ _id: "menu1", isActive: true }),
  });
  guards.getPublicRestaurantOrThrow.mockResolvedValue({
    restaurant: { _id: "r1", name: "Test" },
    availability: { canOrder: true },
  });
  guards.assertRestaurantCanOrder.mockImplementation((availability) => {
    if (!availability?.canOrder) throw new Error("blocked");
  });
  guards.assertRestaurantCanReserve.mockImplementation((availability) => {
    if (!availability?.canReserve) throw new Error("reserve blocked");
  });
});
...
describe("capability gating regressions", () => {
  it("addCartItem blocks inactive/publication/operational/closed-no-policy", async () => {
    ...
    for (const availability of [
      ...
      guards.getPublicRestaurantOrThrow.mockResolvedValueOnce({
        restaurant: { _id: "r1" },
        availability,
      });
      await expect(
  ...
  it("addCartItem passes when closed + allowWhenClosed=true and blocks menu constraints", async () => {
    ...
    await CartMutation.addCartItem(
      null,
      {
        input: {
          restaurantId: "r1",
    ...
    model.MenuItem.findById.mockReturnValueOnce({
      lean: vi.fn().mockResolvedValue({
        _id: "m1",
        restaurantId: "other",
        status: "available",
  ...
  it("updateCartItem delta>0 blocked when canOrder=false, delta<0 still passes", async () => {
    ...
    const cart = {
      ...
      userId: "u1",
      status: "active",
      items: Object.assign([item], { id: vi.fn(() => item) }),
      save: vi.fn(),
      toObject: () => ({ items: [item] }),
    };
    ...
    await expect(
      CartMutation.updateCartItem(
        null,
        { input: { cartId: "c1", itemId: "i1", quantity: 5 } },
        { user: { id: "u1" } },
      ),
    ).rejects.toBeTruthy();
    await expect(
      CartMutation.updateCartItem(
        null,
        { input: { cartId: "c1", itemId: "i1", quantity: 1 } },
        { user: { id: "u1" } },
      ),
    ).resolves.toBeTruthy();
    ...
    item.remove = vi.fn();
    await expect(
      CartMutation.removeCartItem(
        null,
        { input: { cartId: "c1", itemId: "i1" } },
        { user: { id: "u1" } },
      ),
    ).resolves.toBeTruthy();
    model.Cart.findOne.mockReturnValue(sessionQuery(cart));
    await expect(
      CartMutation.clearCart(
        null,
        { input: { cartId: "c1" } },
        { user: { id: "u1" } },
      ),
  ...
  it("addCartItem fails when menuId is not active", async () => {
    const { CartMutation } = await import("../../graphql/resolvers/cart/mutation.js");
    model.Menu.findOne.mockReturnValueOnce({
