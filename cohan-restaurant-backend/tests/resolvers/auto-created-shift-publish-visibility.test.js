...
const modelMocks = vi.hoisted(() => ({
  ...
  Shift: { create: vi.fn(), find: vi.fn(), exists: vi.fn(), countDocuments: vi.fn() },
...
const managerCtx = { user: { id: "manager-1", roles: ["manager"], refRestaurants: ["rest-1"] } };
const staff1Ctx = { user: { id: "staff-1", roles: ["staff"], restaurantForStaff: "rest-1" } };
const staff2Ctx = { user: { id: "staff-2", roles: ["staff"], restaurantForStaff: "rest-1" } };
const otherStaffCtx = { user: { id: "staff-3", roles: ["staff"], restaurantForStaff: "rest-1" } };
...
function publicationForFilter(filter = {}) {
  return db.publications.find((publication) => {
    ...
    if (filter.periodStart?.$lte && new Date(publication.periodStart) > new Date(filter.periodStart.$lte)) return false;
    if (filter.periodEnd?.$gte && new Date(publication.periodEnd) < new Date(filter.periodEnd.$gte)) return false;
    if (filter.periodStart && !filter.periodStart.$lte && new Date(publication.periodStart).getTime() !== new Date(filter.periodStart).getTime()) return false;
    if (filter.periodEnd && !filter.periodEnd.$gte && new Date(publication.periodEnd).getTime() !== new Date(filter.periodEnd).getTime()) return false;
    return true;
  }) || null;
}

function matchesShiftFilter(shift, filter = {}) {
  if (filter.restaurantId && idOf(filter.restaurantId) !== idOf(shift.restaurantId)) return false;
  if (filter.employeeId && idOf(filter.employeeId) !== idOf(shift.employeeId)) return false;
  if (filter.status?.$ne && shift.status === filter.status.$ne) return false;
  if (typeof filter.status === "string" && shift.status !== filter.status) return false;
  if (filter.startTime && !matchesRange(shift.startTime, filter.startTime)) return false;
  return true;
}

function matchesAcknowledgementFilter(ack, filter = {}) {
  if (filter.restaurantId && idOf(filter.restaurantId) !== idOf(ack.restaurantId)) return false;
  if (filter.employeeId && idOf(filter.employeeId) !== idOf(ack.employeeId)) return false;
  if (filter.status && ack.status !== String(filter.status).toLowerCase()) return false;
  if (filter.$and) {
    for (const condition of filter.$and) {
      if (condition.periodEnd?.$gte && new Date(ack.periodEnd) < new Date(condition.periodEnd.$gte)) return false;
      if (condition.periodStart?.$lte && new Date(ack.periodStart) > new Date(condition.periodStart.$lte)) return false;
    }
  }
  return true;
}
...
describe("auto-created shift publish and staff visibility regression", () => {
  beforeEach(() => {
    ...
    modelMocks.ScheduleAcknowledgement.findOneAndUpdate.mockImplementation((filter, update = {}) => {
      let ack = db.scheduleAcknowledgements.find((row) =>
        idOf(row.restaurantId) === idOf(filter.restaurantId) &&
        idOf(row.employeeId) === idOf(filter.employeeId) &&
        idOf(row.schedulePublicationId) === idOf(filter.schedulePublicationId)
      );
      if (!ack) {
        ack = attachSave({ _id: `schedule-ack-${db.nextScheduleAck++}`, ...filter, ...(update.$setOnInsert || {}) });
        db.scheduleAcknowledgements.push(ack);
      }
      Object.assign(ack, update.$set || {});
      return queryResult(ack);
    });
    modelMocks.ScheduleAcknowledgement.find.mockImplementation((filter) => queryResult(db.scheduleAcknowledgements.filter((ack) => {
      if (filter?.restaurantId && idOf(filter.restaurantId) !== idOf(ack.restaurantId)) return false;
      if (filter?.schedulePublicationId && idOf(filter.schedulePublicationId) !== idOf(ack.schedulePublicationId)) return false;
      if (filter?.employeeId?.$in && !filter.employeeId.$in.map(idOf).includes(idOf(ack.employeeId))) return false;
      return true;
    })));
    modelMocks.ScheduleAcknowledgement.findOne.mockImplementation((filter) => Promise.resolve(db.scheduleAcknowledgements.find((ack) =>
      idOf(ack.restaurantId) === idOf(filter.restaurantId) &&
      idOf(ack.employeeId) === idOf(filter.employeeId) &&
      idOf(ack.schedulePublicationId) === idOf(filter.schedulePublicationId)
    ) || null));
    modelMocks.ScheduleAcknowledgement.updateMany.mockResolvedValue({ modifiedCount: 0 });

    modelMocks.ShiftAcknowledgement.findOneAndUpdate.mockImplementation((filter, update = {}) => {
      let ack = db.shiftAcknowledgements.find((row) =>
        idOf(row.shiftId) === idOf(filter.shiftId) && idOf(row.employeeId) === idOf(filter.employeeId)
      );
      if (!ack) {
        ack = attachSave({ _id: `shift-ack-${db.nextShiftAck++}`, ...filter, ...(update.$setOnInsert || {}) });
        db.shiftAcknowledgements.push(ack);
      }
      Object.assign(ack, update.$set || {});
      return queryResult(ack);
    });
    modelMocks.ShiftAcknowledgement.findById.mockImplementation((id) => Promise.resolve(db.shiftAcknowledgements.find((ack) => idOf(ack._id) === idOf(id)) || null));
    modelMocks.ShiftAcknowledgement.findOne.mockImplementation((filter) => Promise.resolve(db.shiftAcknowledgements.find((ack) => {
      if (filter.shiftId && idOf(filter.shiftId) !== idOf(ack.shiftId)) return false;
      if (filter.employeeId && idOf(filter.employeeId) !== idOf(ack.employeeId)) return false;
      return true;
    }) || null));
    modelMocks.ShiftAcknowledgement.find.mockImplementation((filter) => ({
      sort: vi.fn(async () => db.shiftAcknowledgements.filter((ack) => matchesAcknowledgementFilter(ack, filter))),
    }));
    modelMocks.ShiftAcknowledgement.updateMany.mockResolvedValue({ modifiedCount: 0 });
  });
  ...
  it("publishes auto-created draft shifts, creates acknowledgements, exposes staff visibility, and preserves lifecycle guard", async () => {
    ...
    const publication = await mutation.publishSchedule(
      null,
      { input: { restaurantId: "rest-1", periodStart: weekStart.toISOString(), periodEnd: weekEnd.toISOString() } },
      managerCtx,
    );
    ...
    const staffOneShifts = await query.staffShifts(
      null,
      { restaurantId: "rest-1", employeeId: "staff-1", startDate: weekStart.toISOString(), endDate: weekEnd.toISOString() },
      staff1Ctx,
    );

    expect(staffOneShifts).toHaveLength(1);
    expect(staffOneShifts[0]).toEqual(expect.objectContaining({
      employeeId: "staff-1",
      restaurantId: "rest-1",
      shiftType: "morning",
      status: "scheduled",
    }));
    expect(new Date(staffOneShifts[0].startTime).toISOString()).toBe("2026-06-02T06:00:00.000Z");
    expect(new Date(staffOneShifts[0].endTime).toISOString()).toBe("2026-06-02T14:00:00.000Z");
    ...
    await expect(
      mutation.acceptShiftAcknowledgement(null, { id: accepted._id, note: "duplicate" }, staff1Ctx),
    ...
    const declined = await mutation.declineShiftAcknowledgement(
      null,
      { id: db.shiftAcknowledgements[1]._id, reasonCategory: "personal", reason: "  busy family commitment  " },
      staff2Ctx,
    );
    ...
    const declinedForReview = await query.shiftAcknowledgements(
      null,
      { restaurantId: "rest-1", periodStart: weekStart.toISOString(), periodEnd: weekEnd.toISOString(), status: "declined" },
      managerCtx,
    );
    expect(declinedForReview).toHaveLength(1);
    expect(declinedForReview[0]).toEqual(expect.objectContaining({
    ...
    expect(blockedBatch.errors[0]).toEqual(expect.objectContaining({
      index: 0,
      employeeId: "staff-1",
