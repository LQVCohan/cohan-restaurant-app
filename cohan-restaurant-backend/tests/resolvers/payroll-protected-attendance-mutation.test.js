...
vi.mock("../../graphql/resolvers/staff/mutation.js", () => ({ default: staffMutationMock }));
vi.mock("../../models/index.js", () => modelMocks);
vi.mock("../../src/services/payroll/payrollLockGuard.service.js", () => guardMocks);
...
describe("payroll protected staff mutations", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    modelMocks.Shift.findById.mockReturnValue(query({
      _id: ids.shiftId,
    ...
    staffMutationMock.markPayrollItemPaid.mockResolvedValue({ id: "payroll-item-1" });
  });

  it("checks payroll lock before direct shift check-in", async () => {
    const mutation = (await import("../../graphql/resolvers/staff/payrollProtectedAttendance.mutation.js")).default;

    await mutation.checkInShift(null, { shiftId: ids.shiftId }, {}, undefined);

    expect(guardMocks.assertNoLockedPayrollPeriodOverlap).toHaveBeenCalledWith(expect.objectContaining({
      restaurantId: ids.restaurantId,
      employeeId: ids.employeeId,
  ...
  it("does not write attendance when payroll lock guard rejects", async () => {
    guardMocks.assertNoLockedPayrollPeriodOverlap.mockRejectedValueOnce(new Error("PAYROLL_PERIOD_LOCKED"));
    const mutation = (await import("../../graphql/resolvers/staff/payrollProtectedAttendance.mutation.js")).default;

    await expect(mutation.checkOutShift(null, { shiftId: ids.shiftId }, {}, undefined)).rejects.toThrow("PAYROLL_PERIOD_LOCKED");
